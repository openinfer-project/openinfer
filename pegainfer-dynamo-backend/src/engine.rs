//! `PegainferBackend` — adapts pegainfer's `EngineHandle` to Dynamo's
//! `LLMEngine`, so a pure-Rust pegainfer worker plugs into a Dynamo frontend +
...
use dynamo_backend_common::AsyncEngineContext;
use dynamo_backend_common::CommonArgs;
use dynamo_backend_common::DynamoError;
use dynamo_backend_common::EngineConfig;
use dynamo_backend_common::GenerateContext;
use dynamo_backend_common::KvEventPublisher;
use dynamo_backend_common::KvEventSource;
use dynamo_backend_common::LLMEngine;
use dynamo_backend_common::LLMEngineOutput;
use dynamo_backend_common::LLMEngineOutputExt;
use dynamo_backend_common::LlmRegistration;
use dynamo_backend_common::MetricsBindings;
use dynamo_backend_common::MetricsCtx;
use dynamo_backend_common::OnPublisherReady;
use dynamo_backend_common::OnSnapshotPublisherReady;
use dynamo_backend_common::PreprocessedRequest;
use dynamo_backend_common::SnapshotPublisher;
use dynamo_backend_common::WorkerConfig;
use dynamo_backend_common::usage;
use futures::stream::BoxStream;
use pegainfer_engine::engine::EngineHandle;
use pegainfer_engine::engine::GenerateRequest;
use pegainfer_engine::engine::KvBlockEvent;
...
use pegainfer_qwen3::DEFAULT_GPU_MEMORY_UTILIZATION;
...
#[command(
    ...
    about = "pegainfer (Qwen3) backend worker for a Dynamo frontend + KV router."
)]
struct Args {
    #[command(flatten)]
    common: CommonArgs,
    ...
    fn from_parsed(args: Args) -> Result<(Self, WorkerConfig), DynamoError> {
        let served_model_name = args.served_model_name.clone().unwrap_or_else(|| {
            args.model_path
                ...
                .unwrap_or_else(|| "pegainfer".to_string())
        });
        ...
        // and zero-cost, exactly like plain single-machine pegainfer.
...
#[async_trait]
impl LLMEngine for PegainferBackend {
    async fn start(&self, _worker_id: u64) -> Result<EngineConfig, DynamoError> {
        if self.handle().is_some() {
            return Err(convert::engine_shutdown(
                "pegainfer backend already started",
            ));
        }
        ...
        *self.cancel.lock().expect("cancel mutex poisoned") = CancellationToken::new();
        ...
        let handle =
            tokio::task::spawn_blocking(move || pegainfer_qwen3::launch(&model_path, launch))
                .await
        ...
        *self.handle() = Some(handle);
        ...
        Ok(EngineConfig {
            ...
            llm: Some(LlmRegistration {
                ...
                // pegainfer uses no Dynamo-handshake KV transport.
                bootstrap_host: None,
                bootstrap_port: None,
            }),
        })
    ...
    async fn generate(
        ...
        // Per-request private channel + abort reason. pegainfer's scheduler
        ...
        let req = GenerateRequest {
            ...
            // slot stays None), so pin 0 rather than make pegainfer pay the
        ...
        if handle.submit(req).is_err() {
            return Err(convert::engine_shutdown(
                "pegainfer engine is not accepting requests",
            ));
        }
    ...
    async fn cleanup(&self) -> Result<(), DynamoError> {
        ...
        tracing::info!("pegainfer backend: cleanup complete");
        Ok(())
    }
...
/// Forward pegainfer's live KV load to the Dynamo KV router.
///
/// pegainfer's scheduler is a plain OS thread that writes a lock-free `watch`
...
/// Forward pegainfer's neutral KV block events to the Dynamo KV router.
///
...
mod tests {
    ...
    fn served_model_name_defaults_to_dir_name() {
        let (backend, config) = PegainferBackend::from_parsed(
    ...
    fn explicit_served_model_name_and_common_args_flow_through() {
        let (_backend, config) = PegainferBackend::from_parsed(
            Args::try_parse_from([
                "bin",
                "--model-path",
                "/m",
    ...
    fn no_cuda_graph_flag_disables_capture() {
        let (backend, _) = PegainferBackend::from_parsed(
    ...
    // green: set `PEGAINFER_TEST_MODEL_PATH=/path/to/Qwen3-4B`.

    use std::time::Duration;

    use dynamo_backend_common::FinishReason;
    use dynamo_backend_common::SamplingOptions;
    use dynamo_backend_common::StopConditions;
    ...
    fn test_model_path() -> Option<String> {
        let p = std::env::var("PEGAINFER_TEST_MODEL_PATH")
            .unwrap_or_else(|_| "models/Qwen3-4B".to_string());
        if std::path::Path::new(&p).exists() {
            Some(p)
        } else {
            eprintln!("skipping GPU test: no model at {p} (set PEGAINFER_TEST_MODEL_PATH)");
            None
        }
    }

    fn test_backend(model_path: &str) -> PegainferBackend {
        let args =
            Args::try_parse_from(["bin", "--model-path", model_path]).expect("parse test args");
        PegainferBackend::from_parsed(args)
    ...
    /// so it covers the pegainfer→watch wiring without needing a live router.
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
    ...
    /// this worker. That closes the loop that source review opened: pegainfer's
    ...
    /// demand: `PEGAINFER_TEST_MODEL_PATH=… cargo test --release --bins -- --ignored`.
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
    ...
    async fn satisfies_dynamo_llmengine_contract() {
        ...
        dynamo_backend_common::testing::run_conformance(factory)
            .await
            .expect("pegainfer backend satisfies the Dynamo LLMEngine contract");
    }
}

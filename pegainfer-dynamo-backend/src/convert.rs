//! Pure translation between Dynamo's request/response protocol and pegainfer's
//! engine contract.
//!
//! Functional core: every function here is a total function over plain data —
//! no I/O, no async, no GPU. The branchy mapping logic that turns pegainfer
...
use dynamo_backend_common::CompletionUsage;
use dynamo_backend_common::ComponentSnapshot;
...
use dynamo_backend_common::chunk;
...
use dynamo_kv_router::protocols::KvCacheStoreData;
use dynamo_kv_router::protocols::KvCacheStoredBlockData;
use dynamo_kv_router::protocols::LocalBlockHash;
use pegainfer_engine::engine::FinishReason as EngineFinishReason;
...
/// Map Dynamo `SamplingOptions` onto pegainfer's `SamplingParams`.
///
/// pegainfer's sampler is deliberately small — temperature / top-k / top-p /
...
/// `StopConditions` in Dynamo but on `SamplingParams` in pegainfer.
...
/// Outcome of mapping a single pegainfer `TokenEvent` into the Dynamo stream.
pub enum Mapped {
...
/// Translate one pegainfer `TokenEvent` into at most one Dynamo stream item.
///
    ...
    match event {
        ...
        TokenEvent::Finished {
            ...
            match finish_reason {
                ...
                // pegainfer signals real errors via TokenEvent::Error; a
        ...
        // pegainfer reports a cache hit. Carry it out so the terminal usage can
...
/// This is the only signal pegainfer gives the Dynamo frontend's KV-hit metric.
...
/// Build the Dynamo router/Prometheus snapshot from pegainfer's live KV load.
///
...
/// is the publisher's monotonic counter; `dp_rank` is 0 because each pegainfer
/// worker process owns exactly one rank. Multimodal extra-info and absolute
/// `start_position` have no pegainfer analogue and are left `None`.
...
pub fn stream_incomplete() -> DynamoError {
    DynamoError::builder()
        ...
        .message("pegainfer engine closed the token channel before finishing")
        .build()
}
...
mod tests {
    ...
    fn stored_event_renames_hashes_into_dynamo_newtypes() {
        use pegainfer_engine::engine::KvStoredBlock;

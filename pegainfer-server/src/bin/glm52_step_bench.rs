...
use pegainfer::scheduler::SchedulerHandle;
use pegainfer::scheduler::SchedulerRequest;
use pegainfer::scheduler::TokenEvent;
use pegainfer::scheduler::TokenSink;
...
fn main() -> Result<()> {
    ...
    let moe_topo: pegainfer_glm52::Glm52MoeTopo = cli.moe_topo.parse().context("--moe-topo")?;
    ...
    let handle = pegainfer_glm52::launch(
        &cli.model_path,
        pegainfer_glm52::Glm52LaunchOptions {
            tp_size,
            dp_size,
            drafter: pegainfer_glm52::Glm52Drafter::None,
...
fn measure_bucket(handle: &SchedulerHandle, bucket: usize, cli: &Cli) -> Result<Vec<Duration>> {
    let moe_topo: pegainfer_glm52::Glm52MoeTopo = cli
        .moe_topo
        .parse()
...
fn bench_concurrency(bucket: usize, moe_topo: pegainfer_glm52::Glm52MoeTopo) -> usize {
    if is_tensor_replicated_moe(moe_topo) {
...
fn is_tensor_replicated_moe(moe_topo: pegainfer_glm52::Glm52MoeTopo) -> bool {

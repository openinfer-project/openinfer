...
pub(crate) struct Args {
    ...
    #[cfg(feature = "qwen3")]
    #[arg(long, default_value_t = pegainfer_qwen3::DEFAULT_GPU_MEMORY_UTILIZATION)]
    ...
    #[cfg(feature = "qwen3")]
    #[arg(long, default_value_t = (pegainfer_qwen3::DEFAULT_KV_CACHE_MEMORY_MARGIN_BYTES >> 20) as usize)]
    ...
    #[cfg(feature = "qwen3")]
    #[arg(long, default_value_t = pegainfer_qwen3::DEFAULT_KV_PAGE_SIZE)]
...
/// [`pegainfer_qwen3::DecodeOverlap`] together with `--decode-sm-pct`.
#[derive(Clone, Copy, Debug, ValueEnum)]
...
impl CliDecodeOverlap {
    #[cfg(feature = "qwen3")]
    pub(crate) fn resolve(self, decode_sm_pct: u32) -> pegainfer_qwen3::DecodeOverlap {
...
impl CliQwen35SchedulerPolicy {
    #[cfg(feature = "qwen35")]
    pub(crate) fn resolve(self) -> pegainfer_qwen35::Qwen35SchedulerPolicy {
        match self {
            Self::Off => pegainfer_qwen35::Qwen35SchedulerPolicy::Off,
            Self::Auto => pegainfer_qwen35::Qwen35SchedulerPolicy::Auto,
        }
    }
}
    ...
    pub(crate) fn validate(
        ...
        if matches!(model_type, ModelType::Qwen35) {
            if let Some(max_batch) = self.max_batch {
                if !(1..=pegainfer_qwen35::MAX_DECODE_BATCH).contains(&max_batch) {
                    bail!(
        ...
        if matches!(model_type, ModelType::Glm52) {
            ...
            let moe_topo: pegainfer_glm52::Glm52MoeTopo = self
                .moe_topo
                .parse()
            ...
            if self.glm52_prefill_only {
                if !matches!(moe_topo, pegainfer_glm52::Glm52MoeTopo::Tp4) {
            ...
            if self.glm52_prefill_chunk_size == 0
                || !self
                    .glm52_prefill_chunk_size
                    .is_multiple_of(pegainfer_glm52::GLM52_PREFILL_CHUNK_ALIGN)
            {
                bail!(
                    "--glm52-prefill-chunk-size must be a positive multiple of {}, got {}",
                    pegainfer_glm52::GLM52_PREFILL_CHUNK_ALIGN,
            ...
            if self.glm52_native_mtp
                && !matches!(
                    moe_topo,
                    pegainfer_glm52::Glm52MoeTopo::Ep4
                        | pegainfer_glm52::Glm52MoeTopo::Ep8
                        | pegainfer_glm52::Glm52MoeTopo::Ep16
                        | pegainfer_glm52::Glm52MoeTopo::Ep32
                        | pegainfer_glm52::Glm52MoeTopo::Ep64
                )
                && !(self.glm52_prefill_only
                    && matches!(moe_topo, pegainfer_glm52::Glm52MoeTopo::Tp4))
            {
...
mod tests {
    ...
    fn qwen35_accepts_tp_size() {
        let (args, provided) =
            parse_with_provided(&["pegainfer", "--tp-size", "2", "--cuda-graph=false"]);
    ...
    fn qwen35_defaults_scheduler_policy_off() {
        let (args, provided) = parse_with_provided(&["pegainfer"]);
        args.validate(ModelType::Qwen35, &provided)
    ...
    fn qwen35_accepts_scheduler_policy_off() {
        let (args, provided) =
            parse_with_provided(&["pegainfer", "--qwen35-scheduler-policy", "off"]);
        args.validate(ModelType::Qwen35, &provided)
    ...
    fn qwen35_accepts_non_bucket_scheduler_max_batch() {
        let (args, provided) = parse_with_provided(&["pegainfer", "--max-batch", "5"]);
        args.validate(ModelType::Qwen35, &provided)
    ...
    fn qwen35_rejects_zero_scheduler_max_batch() {
        let (args, provided) = parse_with_provided(&["pegainfer", "--max-batch", "0"]);
        let err = args
            .validate(ModelType::Qwen35, &provided)
    ...
    fn qwen3_accepts_graph_png_dump() {
        let (args, provided) =
            parse_with_provided(&["pegainfer", "--dump-graph-png", "decode.png"]);
    ...
    fn glm52_accepts_native_mtp_on_ep8() {
        let (args, provided) = parse_with_provided(&["pegainfer", "--glm52-native-mtp"]);
    ...
    fn glm52_accepts_native_mtp_on_ep4() {
        let (args, provided) =
            parse_with_provided(&["pegainfer", "--glm52-native-mtp", "--moe-topo", "ep4"]);
        assert!(args.glm52_native_mtp);
        args.validate(ModelType::Glm52, &provided)
    ...
    fn glm52_native_mtp_rejects_tp_topology() {
        let (args, provided) =
            parse_with_provided(&["pegainfer", "--glm52-native-mtp", "--moe-topo", "tp4"]);
        let error = args
            .validate(ModelType::Glm52, &provided)
    ...
    fn glm52_rejects_non_dp8_for_ep8() {
        let (args, provided) = parse_with_provided(&["pegainfer", "--dp-size", "1"]);
        let error = args
            .validate(ModelType::Glm52, &provided)
    ...
    fn glm52_accepts_tp4_dp1() {
        let (args, provided) =
            parse_with_provided(&["pegainfer", "--moe-topo", "tp4", "--tp-size", "4"]);
        args.validate(ModelType::Glm52, &provided)
    ...
    fn glm52_tp4_rejects_omitted_tp_size() {
        let (args, provided) = parse_with_provided(&["pegainfer", "--moe-topo", "tp4"]);
        let error = args
            .validate(ModelType::Glm52, &provided)
    ...
    fn glm52_ep8_rejects_tp4_tp_size() {
        let (args, provided) = parse_with_provided(&["pegainfer", "--tp-size", "4"]);
        let error = args
            .validate(ModelType::Glm52, &provided)
    ...
    fn glm52_rejects_unknown_moe_topo() {
        let (args, provided) = parse_with_provided(&["pegainfer", "--moe-topo", "tp2"]);
        let error = args
            .validate(ModelType::Glm52, &provided)
    ...
    fn glm52_accepts_ep4_default_dp4() {
        let (args, provided) = parse_with_provided(&["pegainfer", "--moe-topo", "ep4"]);
        args.validate(ModelType::Glm52, &provided)
    ...
    fn glm52_ep4_rejects_non_dp4() {
        let (args, provided) =
            parse_with_provided(&["pegainfer", "--moe-topo", "ep4", "--dp-size", "8"]);
        let error = args
            .validate(ModelType::Glm52, &provided)
    ...
    fn glm52_prefill_only_accepts_tp4_defaults() {
        ...
        assert_eq!(
            ...
            pegainfer_glm52::GLM52_DEFAULT_PREFILL_CHUNK_SIZE
        );
    }
    ...
    fn glm52_prefill_chunk_requires_mode_and_page_alignment() {
        let (args, provided) =
            parse_with_provided(&["pegainfer", "--glm52-prefill-chunk-size", "16384"]);
        let error = args
            .validate(ModelType::Glm52, &provided)

...
impl DeepSeekV4DirectGenerator {
    ...
    #[cfg(feature = "pplx-ep")]
    pub fn enable_pplx(&self, ep_backends: Vec<openinfer_comm::EpBackend>) -> Result<()> {
...
pub fn start_engine(model_path: &Path, options: EngineLoadOptions) -> Result<EngineHandle> {
    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            ...
            #[cfg(feature = "pplx-ep")]
            if std::env::var("OPENINFER_DSV4_PPLX").is_ok_and(|v| v != "0" && !v.is_empty()) {
                info!("OPENINFER_DSV4_PPLX set; building pplx EP backends");
                let ep_shape = openinfer_comm::bootstrap::EpModelShape {
                ...
                match openinfer_comm::bootstrap::build_intra_node_backends(
                    ep_shape,
                    &(0..8).collect::<Vec<_>>(),
                    generator.runtime.thread_placement(),
                    openinfer_comm::bootstrap::PplxBootstrapParams::default(),
                ) {
...
mod tests {
    ...
    fn batch_decode_logits_match_two_single_decode_rows() -> Result<()> {
        let model_path = env::var_os("OPENINFER_TEST_MODEL_PATH")
            .map(PathBuf::from)

...
use pegainfer_kv_offload::QueryHandle;
use pegainfer_kv_offload::QueryOutcome;
use pegainfer_kv_offload::SaveHandle;
...
#[derive(Deserialize)]
struct PegaInferPdEnvelope {
    pegainfer_pd: NativeMtpEnvelope,
}
...
pub(super) fn native_mtp_handoff(
    ...
    let version = envelope.pegainfer_pd.version;
    anyhow::ensure!(
        version == 2,
        "unsupported pegainfer P/D metadata version {version}"
    );
    let handoff = envelope.pegainfer_pd.native_mtp;
    anyhow::ensure!(
...
/// Native PegaInfer P/D restore. Full pages use kvbm lineage hashes; the
...
mod tests {
    ...
    use pegainfer_kv_offload::QueryHit;
        ...
        let mut state = HostRestoreState {
            pending: Some(PendingHostRestore {
                ...
                op: PendingHostOp::Load {
                    probe,
                    reservation,
                    handle: LoadHandle::settled(Err(pegainfer_kv_offload::EngineError::Storage(
                        "injected".into(),
                    ))),
                },
            }),
    ...
    fn native_pd_rejects_v1_metadata() {
        ...
        assert!(
            error
                .to_string()
                .contains("unsupported pegainfer P/D metadata version 1"),
            "{error:#}"
        );
    }

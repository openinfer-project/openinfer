...
CUresult kimi_int4_grouped_w1_w3_cuda(
  ...
  if (expert_major_hidden == nullptr || w1_packed == nullptr || w1_scale == nullptr ||
      w1_shape == nullptr || w3_packed == nullptr || w3_scale == nullptr ||
      w3_shape == nullptr || expert_indptr == nullptr || gate_out == nullptr ||
      up_out == nullptr) {
    return CUDA_ERROR_INVALID_VALUE;
  }
  if (routed_tokens < 0 || hidden_dim != kKimiHiddenDim ||
      intermediate_dim != kKimiExpertIntermediateDim || local_experts != kKimiLocalExperts ||
      group_size != kKimiInt4GroupSize) {
    return CUDA_ERROR_INVALID_VALUE;
  }
  if (routed_tokens == 0) return CUDA_SUCCESS;
  // Production path is CUTLASS C++ AOT based on example 69. A serial
  // dequant-dot kernel is intentionally not provided here.
  return CUDA_ERROR_NOT_SUPPORTED;
}

CUresult kimi_int4_grouped_w2_swiglu_cuda(
    const __nv_bfloat16* gate,
    const __nv_bfloat16* up,
    const uint8_t* w2_packed,
    const __nv_bfloat16* w2_scale,
    const int32_t* w2_shape,
    const uint32_t* expert_indptr,
    ...
    cudaStream_t stream) {
  (void)stream;
  if (gate == nullptr || up == nullptr || w2_packed == nullptr || w2_scale == nullptr ||
      w2_shape == nullptr || expert_indptr == nullptr || expert_output == nullptr) {
    return CUDA_ERROR_INVALID_VALUE;
  }
  if (routed_tokens < 0 || intermediate_dim != kKimiExpertIntermediateDim ||
      hidden_dim != kKimiHiddenDim || local_experts != kKimiLocalExperts ||
      group_size != kKimiInt4GroupSize) {
    return CUDA_ERROR_INVALID_VALUE;
  }
  if (routed_tokens == 0) return CUDA_SUCCESS;
  // Production path is CUTLASS C++ AOT W2 after the external SwiGLU scratch.
  // A serial dequant-dot kernel is intentionally not provided here.
  return CUDA_ERROR_NOT_SUPPORTED;
}

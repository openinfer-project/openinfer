---
title: 4.6. Green Contexts — CUDA Programming Guide
source: https://docs.nvidia.com/cuda/cuda-programming-guide/04-special-topics/green-contexts.html
fetched: 2026-06-15T08:02:45Z
---
...
[Figure 45](#id2) illustrates such an example. Assume an application where two independent kernels A and B run on two different non-blocking CUDA streams.
...
[![Green Contexts Motivation](../_images/green_contexts_motivation.png)](../_images/green_contexts_motivation.png)
...
As [Figure 46](#resource-split-by-count) highlights, the user requests to split the `input` SM-type device resource into `*nbGroups` homogeneous groups with `minCount` SMs each.
...
[![SM Resource Split using cudaDevSmResourceSplitByCount API](../_images/green_contexts_resource_split_by_count.png)](../_images/green_contexts_resource_split_by_count.png)
...
In a successful split, as shown in [Figure 47](#resource-split), each resource in the `result` can have a different number of SMs, but never zero SMs.

[![SM Resource Split using cudaDevSmResourceSplit API](../_images/green_contexts_resource_split.png)](../_images/green_contexts_resource_split.png)
...
[Figure 48](#green-contexts-ncu-mask) depicts these resources for an example with two green contexts provisioned with 112 and 16 SMs respectively, with no SM overlap across them.
...
[![Green Contexts Resources in Nsight Compute](../_images/green_contexts_ncu_mask.png)](../_images/green_contexts_ncu_mask.png)
...
[Figure 49](#green-contexts-nsys-example-no-gcs-with-prio) shows this scenario in an Nsight Systems report.
...
[![Nsight Systems timeline without green contexts](../_images/green_contexts_nsys_example_no_GCs_with_prio.png)](../_images/green_contexts_nsys_example_no_GCs_with_prio.png)
...
As [Figure 50](#green-contexts-nsys-example-w-gcs) shows, the critical kernel can now start almost instantaneously, as there are SMs only Green Context 3 can use.
...
[![Nsight Systems timeline with green contexts](../_images/green_contexts_nsys_example_w_GCs.png)](../_images/green_contexts_nsys_example_w_GCs.png)

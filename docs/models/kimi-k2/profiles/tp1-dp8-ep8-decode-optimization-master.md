...
| Surface | Field | Meaning at the target load |
|---|---|---|
| `kimi_tp1_pplx_decode_bench` | `--active-rows 8` | Single DP-rank local operator shape. This is the master-table anchor and corresponds to global `bs~=64` under DP8. |
| `kimi_kernel_report trace/run` runtime source | `--batch-size 64 --tp-world 1 --dp-world 8 --ep-backend pplx` | Global request count submitted to the scheduler. Using `--batch-size 8` here is only global bs8 and does **not** cover the target per-rank bs8 case. |
| Serving / token A/B gates | concurrency / request count `64` | Global bs64, distributed by the TP1 DP8 scheduler into about 8 rows per DP rank. |
...
|---|---|---:|---:|---:|---|---|
| `decode.moe.shared_gate_up` | `shared_gate_up_report.md` | `1.818 ms` | `1.505 ms` | `1.21x` | memory | `target/kernel_reports/kimi-k2/tp1-pplx-decode-bench-cublaslt-bs3-bs8.json`, per-rank `active_rows=8,ctx=1` |
| `decode.attention.o_proj` | `attention_o_proj_report.md` | `2.715 ms` | `2.374 ms` | `1.14x` | memory | `target/kernel_reports/kimi-k2/tp1-pplx-decode-bench-o-proj-cublaslt-bs8.json`, `profile/kimi-attention-o-proj-h20-cublaslt/`, per-rank `active_rows=8,ctx=1` |
| `decode.attention.absorb_q_nope` | `attention_absorb_q_nope_report.md` | `973.6 us` | `748.5 us` | `1.30x` | memory | `target/kernel_reports/kimi-k2/tp1-pplx-decode-bench-mla-cublaslt-bs8-lazy128.json`, `profile/kimi-mla-cublaslt-h20/`, per-rank `active_rows=8,ctx=1` |
| `decode.attention.v_up` | `attention_v_up_report.md` | `781.0 us` | `738.5 us` | `1.06x` | memory | `target/kernel_reports/kimi-k2/tp1-pplx-decode-bench-mla-cublaslt-bs8-lazy128.json`, `profile/kimi-mla-cublaslt-h20/`, per-rank `active_rows=8,ctx=1` |
...
| 10 | attention | `decode.attention.rope_split` | `kimi_mla_rope_split_decode_rt` | 61 | batch=8, local_heads=64, q_head_dim=192, BF16 | 441.8 us | 7.2 us | 54.4 GB/s payload-equivalent | control/elementwise | NCU: `384` CTAs, `0.62` waves/SM, `10.51%` SM, `1.27%` DRAM, `77.03%` no-eligible; standalone stopped | measured; NCU done |
...
| 12 | attention | `decode.attention.paged_kv_append` | `kimi_mla_paged_kv_append` | 61 | elems=4608, BF16, arena_rows=8, production page stride=128 pages/request | 431.0 us | 7.07 us | 2.63 GB/s payload-equivalent | control/tiny-grid | NCU: `78` CTAs, `0.12` waves/SM, `1.50%` SM, `0.09%` DRAM, `97.63%` no-eligible; standalone stopped | measured; NCU done |
| 13 | attention | `decode.attention.flashinfer_mla_decode` | `kimi_flashinfer_batch_decode_mla_rt` | 61 | elems=512 at `ctx=1`, BF16 paged MLA KV | 624.6 us | 10.2 us | 0.11 TF/s (`ctx=8192`: 2.85 TB/s payload baseline) | mixed/ctx-sensitive | ctx1 4.4% HBM; synthetic ctx8192 baseline `103.5ms`; bench-only p128 partition `52.5ms` (`~1.97x`) with decode grid 128 CTAs plus merge; production-cap fixed-grid p32 partition improves ctx1024/2048 `13.37/26.24ms -> 7.31/13.85ms`; synthetic max abs diff `0.001953`; production adoption rejected because global bs64 TP1/DP8/PPLX runtime gates failed before the partition branch | measured; selected NCU metrics; partition probe not adopted |
...
| 16 | attention | `decode.attention.post_attn_add_norm` | `fused_add_rms_norm_round_batch` | 61 | rows=8, hidden=7168, BF16 add+RMSNorm round | 530.0 us | 8.7 us | 79.2 GB/s payload-equivalent | control/tiny-grid | NCU: `8` CTAs, `0.05` waves/SM, `2.29%` SM, `1.11%` DRAM; standalone stopped | measured; NCU done |
...
| 26 | moe_pplx_compute | `decode.moe.pplx_swiglu` | `kimi_marlin_w13_swiglu_pplx` | 60 | trace replay p95: work rows=224, launch rows=848, gate_up=4096, out=2048, BF16; p50/max padded=96/336 | 759.7 us | 12.7 us | 217.4 GB/s payload-equivalent | compute/elementwise | - | measured trace replay |
...
| 28 | moe_pplx_compute | `decode.moe.residual_add_scaled` | `kimi_residual_add_scaled_f32` | 60 | rows=8, hidden=7168, BF16 + F32 routed, scale=2.827 | 408.3 us | 6.8 us | 84.3 GB/s payload-equivalent | control/elementwise | NCU: `224` CTAs, `0.36` waves/SM, `8.37%` SM, `3.33%` DRAM, `91.25%` no-eligible; standalone stopped | measured; NCU done |
...
Measured rows sorted by step latency at per-rank `active_rows=8,ctx=1`:
...
| 9 | PPLX `decode.moe.pplx_swiglu` | 759.7 us | `pplx_swiglu_report.md`: trace replay p95 is `12.7us/call`; NCU says `swiglu_w13_pplx_kernel` is elementwise/SFU rather than HBM-bound (`10.62us`, `6784` CTAs, `55.4%` SM, `6.3%` DRAM). Standalone tuning is stopped; future work needs W13/W2 fusion or a tighter route-aware launch bound. |
...
All non-communication local compute rows now have H20 provider benches. `decode.attention.rope_split`, `decode.attention.paged_kv_append`, and `decode.moe.residual_add_scaled` have selected H20 NCU coverage and are stopped as standalone kernels. PPLX routed local compute has a synthetic stress provider plus a trace replay provider for local routing/W13/SwiGLU/W2; an all-rank harness is still required before turning those rows into EP transport or end-to-end serving-throughput claims. EP communication rows remain excluded from optimization but must stay visible in this table.
...
|---:|---:|---|---|---|
| 1 | 2 | `decode.dense.gate_up` | Event roofline only; no production NCU for the single dense-layer GEMM. | Called once per decode step and already reports `74.5%` HBM by event roofline, so NCU is for completeness or a concrete cuBLAS scheduling gap, not an urgent bottleneck. |
| 2 | 4 | `decode.dense.down` | Event roofline only; no production NCU for the single dense-layer GEMM. | Called once per decode step and reports `64.6%` HBM by event roofline; NCU can confirm whether there is any library scheduling gap. |
| 3 | 3 | `decode.dense.swiglu` | Event timing + source geometry only. | One-call elementwise row at `7.8us`; profile only if closing the standalone report inventory. |
| 4 | 5 | `decode.dense.residual_add` | Event timing + source geometry only. | One-call elementwise row at `6.8us`; profile only if closing the standalone report inventory or evaluating dense down epilogue fusion. |
...
`decode.attention.flashinfer_mla_decode` is parked rather than queued for more NCU: baseline and partition selected NCU exist, the bench-only partition signal is strong, and the current blocker is production global-bs64 TP1/DP8/PPLX runtime correctness rather than missing kernel counters.
...
| 11 | `attention_paged_kv_append_report.md` | Exists; provider now uses production page stride, H20 bench is measured, and production-metadata selected NCU confirms tiny-grid/control behavior (`78` CTAs, `0.12` waves/SM, `1.50%` SM, `0.09%` DRAM, `97.63%` no-eligible). Standalone tuning is stopped. |
...
| 15 | `attention_flashinfer_mla_decode_report.md` | Exists; H20 ctx sweep says `ctx=1` is launch/control-heavy (`10.24us/call`) while synthetic `ctx=8192` baseline is `1.697ms/call`. Bench-scoped p128 `partition_kv` improves synthetic `ctx=4096/8192` by about `1.95x/1.97x`, selected NCU confirms higher decode parallelism plus merge overhead, and synthetic output-equivalence passes. Under current production cap, graph-safe fixed-grid p32 improves H20 `ctx=1024/2048` by `1.83x/1.89x`, but production TP1/DP8/PPLX runtime gates failed before validating the branch, so adoption is rejected for this pass. |
...
| 17 | `attention_rope_split_report.md` | Exists; `rope_split_decode_kernel` is a control/elementwise MLA prep helper (`441.8us/step`, `7.24us/call`). Selected H20 NCU confirms low utilization (`384` CTAs, `0.62` waves/SM, `10.51%` SM, `1.27%` DRAM, `77.03%` no-eligible), so standalone tuning is stopped and only launch-removing MLA prep fusion remains plausible. |
...
| 19 | `attention_post_attn_add_norm_report.md` | Exists; exact-preserving fused add + RMSNorm round is a tiny-grid/control row (`8` CTAs, `896` threads/CTA, `28,784B` dynamic smem, `8.65-8.69us/call`). H20 NCU confirms `0.05` waves/SM, `2.29%` SM throughput, and `1.11%` DRAM throughput; standalone tuning is stopped unless a downstream prologue fusion preserves BF16 rounding and clears the full-bench gate. |
...
| 25 | `pplx_residual_add_scaled_report.md` | Exists; PPLX post-combine residual scaled add is an exact-preserving elementwise helper (`408.3-410.1us/step`, `6.81-6.83us/call`). Selected H20 NCU confirms low utilization (`224` CTAs, `0.36` waves/SM, `8.37%` SM, `3.33%` DRAM, `91.25%` no-eligible), so standalone tuning is stopped and only launch-removing fusion remains plausible. |
| 26 | `pplx_build_marlin_routing_report.md` | Exists; PPLX Marlin routing metadata builder is a one-block control row (`1 x 64`, `9.87us/call` p95 replay, NCU `0.04%` DRAM), so standalone tuning is stopped and only launch-removing route-aware Marlin work remains plausible. |
| 27 | `pplx_swiglu_report.md` | Exists; PPLX routed SwiGLU is elementwise/SFU (`12.66us/call` p95 replay, NCU `6784 x 256`, `55.4%` SM, `6.3%` DRAM), so standalone tuning is stopped and only W13/W2 fusion or route-aware launch-bound work remains plausible. |

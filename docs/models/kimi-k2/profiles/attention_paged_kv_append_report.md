...
> **TL;DR:** `decode.attention.paged_kv_append` has a TP1 PPLX measured provider using the production decode arena page shape (`page_size=16`, `128` pages/request, page base `request_idx * 128`) and reports `BoundKind::Control`, so the bench does not fabricate an HBM peak percentage for this tiny append. H20 production-metadata bench is `7.07us/call`, `431.0us` per 61 attention layers at `ctx=1`; ctx sweep through the arena capacity (`1/128/1024/2048`) stays `7.07-7.36us/call`. Production-metadata NCU confirms this is tiny-grid/control (`78` CTAs, `0.12` waves/SM, `1.50%` SM, `0.09%` DRAM, `4.40GB/s`, `97.63%` no-eligible). Stop standalone tuning; only launch-removing MLA cache-prep fusion remains plausible.
>
...
| `wiki/patterns/low-sm-utilization.md` | Low SM utilization often comes from a grid/workload too small for the GPU, tail effects, or scheduling overhead; persistent scheduling only helps if there is enough useful work to schedule. | Production-metadata NCU matches this pattern: `78` CTAs on `78` SMs but only `0.12` waves/SM and very low useful throughput. |
...
Production-metadata selected NCU:
...
| Memory throughput | `4.40GB/s`, `0.36%` memory throughput |
| DRAM throughput | `0.09%` |
| Compute throughput | `1.50%` |
| Achieved occupancy | `8.71%` |
| L1/TEX / L2 hit rate | `26.92%` / `63.33%` |
| Scheduler no eligible | `97.63%` |
| Top rule | Grid/workload too small; issue slot utilization local speedup estimate `97.63%` |
...
Diagnosis: the kernel writes only one compressed KV row plus KPE row per arena slot. The provider uses `nnz=8`, so useful payload is tiny even though FlashInfer launches a grid sized to the SM count. The timing is mostly launch/control/scheduler overhead and metadata work; neither H20 tensor compute nor HBM bandwidth is the active ceiling.

2026-06-01 H20 rerun status: `h20-100` completed the production-metadata selected NCU after the same tmpfs target binary used for the MLA partition experiments. The remote `.ncu-rep` is `/dev/shm/kimi-kv-append-prod-ncu/reports/kv_append_selected.ncu-rep`; parsed CSV is copied locally to `profile/kimi-mla-paged-kv-append-prod-h20/analysis/kv_append_details.csv`. The `.ncu-rep` copy was skipped after repeated rsync timeouts, but the exported details CSV is the source of the metrics above.
...
| Standalone kernel rewrite | Not attempted. Production-metadata NCU says the row is not saturating memory or compute, and the aggregate cost is below the existing top-10 local compute rows. | Reject. |
...
Keep FlashInfer `kimi_mla_paged_kv_append` as the production provider. Do not claim the row has reached any H20 memory peak; production-metadata NCU confirms it is control/tiny-grid work. Stop standalone tuning. Reopen only if a nearby MLA cache-prep fusion can remove the launch while preserving cache layout and full TP1 PPLX correctness.

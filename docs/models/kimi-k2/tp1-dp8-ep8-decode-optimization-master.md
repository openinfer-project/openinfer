...
|---:|---|---:|---|
| 1 | PPLX `decode.moe.pplx_marlin_w13` | 15.038 ms | `pplx_marlin_compute_report.md`: current focus. The row now uses H20 trace replay p95 (`recv=67`, `padded=224`, active local experts `28`) rather than the old synthetic `400` padded-row stress point. Replay p50/p95/max per-call latency is `114.5/250.6/368.6us`; p95 is memory-bound at `38.9%` HBM by the H20 roofline. Synthetic NCU still shows the underlying Marlin kernel is single-wave/mixed-pressure (`234` CTAs, `1.0` waves/SM, `58.7%` SM, `34.7%` DRAM), so the next useful profile is NCU on a trace p95/max replay sample. |
| 2 | PPLX `decode.moe.pplx_marlin_w2` | 8.311 ms | `pplx_marlin_compute_report.md`: current focus. H20 trace replay p50/p95/max per-call latency is `66.4/138.5/200.3us`; p95 is memory-bound at `35.4%` HBM. Synthetic NCU still shows a single-wave Marlin kernel (`234` CTAs, `1.0` waves/SM, `56.8%` SM, `32.6%` DRAM); use the replay sample before changing Marlin parameters. |
...
| 7 | `pplx_marlin_compute_report.md` | Exists; synthetic expected-local-route provider measures PPLX W13/SwiGLU/W2/routing without EP transport, and trace replay now consumes H20 route histograms directly. Next artifact should NCU-profile replay p95/max W13/W2 before changing Marlin parameters. |

...
PPLX routed local compute rows use a synthetic expected-local-route provider until the all-rank harness can expose real recv histograms: for `bs=8/rank`, global `bs~=64`, each EP rank is modeled with `64` expected local routes, `400` expected padded rows, and `recv_capacity=848`. EP dispatch/combine transport remains excluded.
...
|---:|---|---:|---|
| 1 | PPLX `decode.moe.pplx_marlin_w13` | 26.186 ms | `pplx_marlin_compute_report.md`: synthetic expected-local-route baseline. Bench roofline is memory-bound at `38.3%` HBM; NCU Marlin kernel shows mixed pressure (`234` CTAs, `1.0` waves/SM, `58.7%` SM, `34.7%` DRAM). Needs all-rank route histogram before kernel optimization. |
| 2 | PPLX `decode.moe.pplx_marlin_w2` | 14.208 ms | `pplx_marlin_compute_report.md`: synthetic expected-local-route baseline. Bench roofline is memory-bound at `35.5%` HBM; NCU Marlin kernel shows mixed pressure (`234` CTAs, `1.0` waves/SM, `56.8%` SM, `32.6%` DRAM). Needs all-rank route histogram before kernel optimization. |
...
| 7 | `pplx_marlin_compute_report.md` | Exists; synthetic expected-local-route provider measures PPLX W13/SwiGLU/W2/routing without EP transport. Needs all-rank route histogram before optimization. |

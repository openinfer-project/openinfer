...
|---|---|---|
| Kimi-specific cuBLASLt shared_gate_up | `target/kernel_reports/kimi-k2/tp1-pplx-decode-bench-cublaslt-bs3-bs8.json`, `bs=8,ctx=1` | `shared_gate_up` `1.818 ms -> 1.505 ms` per 60 calls (`1.21x`); should be committed separately as an `opt(shared_gate_up)` commit after the baseline milestone. |
...
| 2 | `opt(shared_gate_up)` | Kimi-specific cuBLASLt code path, shared_gate_up report updates, and this master table's post-baseline result update. | unrelated bench scaffolding or other kernel changes. |

The current worktree has both baseline work and the first accepted optimization present. Split the commits by staging hunks, not by rewriting the history of the measurements.
...
| 21 | moe_shared | `decode.moe.shared_gate_up` | `gemm_dm_typed_to_hs_graphsafe` | 60 | rows=8, out=4096, in=7168, BF16 | 1.818 ms | 30.3 us | 15.50 TF/s | memory | 40.5% / gap 59.5% | measured |
...
| 3 | `decode.moe.shared_gate_up` | 1.818 ms | Post-baseline cuBLASLt candidate already measures `1.505 ms`; commit separately after baseline milestone. |
...
| 3 | `shared_gate_up_report.md` | Post-baseline cuBLASLt optimization has evidence and needs a clean opt commit after the baseline commit. |

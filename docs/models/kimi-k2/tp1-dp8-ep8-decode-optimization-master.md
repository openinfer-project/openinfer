...
| 25 | moe_pplx_compute | `decode.moe.pplx_marlin_w13` | `kimi_marlin_wna16_pplx_w13_gemm` | 60 | trace replay p95: work rows=224, recv=67, active_experts=28, recv_capacity=848, out=4096, in=7168, WNA16 INT4 weights + BF16 activations; p50/max padded=96/336 | 15.038 ms | 250.6 us | 52.48 TF/s / 1.87 TB/s | memory | 38.9% / gap 61.1% | measured trace replay |
...
| 27 | moe_pplx_compute | `decode.moe.pplx_marlin_w2` | `kimi_marlin_wna16_pplx_w2_gemm` | 60 | trace replay p95: work rows=224, recv=67, active_experts=28, recv_capacity=848, out=7168, in=2048, WNA16 INT4 weights + BF16 activations; p50/max padded=96/336 | 8.311 ms | 138.5 us | 47.48 TF/s / 1.70 TB/s | memory | 35.4% / gap 64.6% | measured trace replay |
...
|---:|---|---:|---|
| 1 | PPLX `decode.moe.pplx_marlin_w13` | 15.038 ms | `pplx_marlin_compute_report.md`: current focus. The row uses H20 trace replay p95 (`recv=67`, `padded=224`, active local experts `28`) rather than the old synthetic `400` padded-row stress point. Replay p50/p95/max per-call latency is `114.5/250.6/368.6us`; p95 is memory-bound at `38.9%` HBM by the H20 roofline. NCU p95: `265.6us`, `234` CTAs, `1.0` waves/SM, `76KB` dynamic shared memory, `18.75%` theoretical occupancy, `58.1%` SM throughput, `14.1%` tensor pipe, `1.75 TB/s` DRAM read, `4.45%` L2 hit. Not at H20 limit; only try variants that lower shared memory / increase waves / improve small-M route grouping. |
| 2 | PPLX `decode.moe.pplx_marlin_w2` | 8.311 ms | `pplx_marlin_compute_report.md`: current focus. H20 trace replay p50/p95/max per-call latency is `66.4/138.5/200.3us`; p95 is memory-bound at `35.4%` HBM. NCU p95: `144.3us`, `234` CTAs, `1.0` waves/SM, `76KB` dynamic shared memory, `18.75%` theoretical occupancy, `55.9%` SM throughput, `13.0%` tensor pipe, `1.60 TB/s` DRAM read, `5.07%` L2 hit. Same conclusion as W13: not HBM-saturated; target occupancy/waves/shared-memory footprint first. |
...
| 7 | `pplx_marlin_compute_report.md` | Exists; synthetic expected-local-route provider measures PPLX W13/SwiGLU/W2/routing without EP transport, trace replay consumes H20 route histograms directly, and p95 NCU says W13/W2 are single-wave/shared-memory-limited rather than HBM-saturated. |

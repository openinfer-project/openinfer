...
| 5 | attention `q_b_proj` | 1.052 ms | NCU done: cuBLAS skinny GEMM `64` blocks/`0.82` waves/SM/`59-61%` DRAM, low L2 hit (`2.7-2.8%`); row 8/9 fusion would need a custom prologue while preserving MLA cache outputs. |

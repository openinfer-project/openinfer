...
| 13 | attention | `decode.attention.flashinfer_mla_decode` | `kimi_flashinfer_batch_decode_mla_rt` | 61 | elems=512 at `ctx=1`, BF16 paged MLA KV | 624.6 us | 10.2 us | 0.11 TF/s (`ctx=8192`: 2.85 TB/s payload) | mixed/ctx-sensitive | ctx1 4.4% HBM; ctx8192 payload ~59% HBM; selected NCU says 32 CTAs / 0.41 waves / 0.87% DRAM | measured; selected NCU metrics |
...
| 28 | moe_pplx_compute | `decode.moe.residual_add_scaled` | `kimi_residual_add_scaled_f32` | 60 | rows=8, hidden=7168, BF16 + F32 routed, scale=2.827 | 408.3 us | 6.8 us | 84.3 GB/s payload-equivalent | control/elementwise | - | measured; production NCU pending |
...
|---:|---:|---|---|---|
| 1 | 13 | `decode.attention.flashinfer_mla_decode` | Full `.ncu-rep` was captured remotely but not retrieved; selected NCU stdout metrics exist (`32` CTAs, `0.41` waves/SM, `254` regs/thread, `0.87%` DRAM). Source-counter stall breakdown is still missing. | Highest long-context impact: `ctx=8192` is `103.5ms/step`; selected NCU points to low-grid/register-limited dispatch, so test FlashInfer planned `partition_kv`. |
...
| 15 | `attention_flashinfer_mla_decode_report.md` | Exists; H20 ctx sweep says `ctx=1` is launch/control-heavy (`10.24us/call`) while `ctx=8192` is `1.697ms/call` by event timing. Selected NCU metrics for `ctx=8192` show `32` CTAs, `0.41` waves/SM, `254` regs/thread, and no DRAM saturation, so the active candidate is a planned FlashInfer `partition_kv` bench path. |
...
| 25 | `pplx_residual_add_scaled_report.md` | Exists; PPLX post-combine residual scaled add is an exact-preserving elementwise helper (`408.3-410.1us/step`, `6.81-6.83us/call`, `224` CTAs, `~84GB/s` payload-equivalent), so standalone tuning is stopped and only launch-removing fusion remains plausible. |

...
- bulk restore 的块注册是 O(blocks) 的 CPU 簿记（~70µs/块），在 scheduler 线程上一次付清会冻结全部流的 token 交付 ~70ms（openinfer [#704](https://github.com/openinfer-project/openinfer/issues/704)，[#705](https://github.com/openinfer-project/openinfer/pull/705) 改为每 tick 64 块分期 + pinned token 回读修复）。诊断教训：GPU 占空比正常但 token 停 = 查 scheduler 线程,别猜带宽。

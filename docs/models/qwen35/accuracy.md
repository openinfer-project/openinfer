...
- Failure semantics: an unreadable/malformed config or a missing committed fixture (`4b`/`9b`/`27b`) panics; a recognized size with no committed fixture (none currently) skips and prints the expected path; an env override pointing at a missing file panics.
- Tolerances are shared across sizes from the 4B calibration until a new size has a green baseline to calibrate against (the `MARGIN_TOL`/`MEAN_TOL`/`P99_TOL` consts); the 9B and 27B floors sit well inside them.

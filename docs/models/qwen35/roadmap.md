...
> **TL;DR:** Qwen3.5-4B is decode-correct and still improving: the decode-tuning refresh improves direct TPOT by `2.1-3.2%`, while vLLM still leads 1024/256 HTTP decode and high-concurrency throughput. Long-prompt HF logits and GSM8K gates cover the old 4096-position RoPE boundary. DFlash now has an opt-in single-active MVP; concurrent speculative throughput remains follow-up work. Remaining structural items are HND prefill staging, prefix-cache design, and the serving-level concurrency gap.
>
...
| DFlash | Opt-in single-active greedy DFlash path behind `--dflash-draft-model-path`; multi-active/logprobs fall back to normal decode, and concurrent throughput uplift is not claimed yet | `docs/models/qwen35/dflash-speculative-decoding.md` |

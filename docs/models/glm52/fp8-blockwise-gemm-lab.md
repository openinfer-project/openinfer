...
- **Tests**: `openinfer-glm52/benches/tests/` CPU-only pytest (never imports torch): registry/manifest/schema invariants, fp8 unit metadata vs production anchors (`whitelisted_linear_shape` / `FP8_GEMM_WORKSPACE_BYTES` / the `extern "C"` signature), and a latch test for the tcgen05 gating keys.

Usage and environment: `openinfer-glm52/benches/README.md`; operating and extension discipline: `kernel-lab-ops.md`.

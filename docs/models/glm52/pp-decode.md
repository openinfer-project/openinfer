...
**TPOT baseline (eager, eight stages serial): ~70 ms median.** Sync-bracketed per-op GPU time (position 8) re-prioritizes the entire optimization. The HBM floor below is **PP-serial** (one H200 active at a time — PP gives no aggregate-BW benefit at bs=1, so the floor is the *sum* of every stage's weight read at one card's ~4.8 TB/s, NOT divided by 8):

| component | time/token | active fp8 read | PP-serial floor | BW eff |
|---|---|---|---|---|
| MoE grouped FP8 GEMMs (W13+W2, 8 active × 75 layers) | ~29 ms | 22.6 GB | 4.72 ms | ~16% |
| MoE glue + shared expert (75 layers) | ~20 ms | 2.8 GB (shared) | 0.59 ms | — |
| MLA (10 fp8 projections + FlashMLA, 78 layers) | ~21 ms | 12.9 GB | 2.68 ms | ~13% |
| dense MLP (3) + lm_head | (in above) | 2.6 GB | 0.54 ms | — |
| **total** | **~70 ms** | **40.9 GB** | **8.53 ms** | **~12%** |

**The 10 ms target is TIGHT, not comfortable — only ~15% headroom over the 8.53 ms floor.** It is reachable *only* if every kernel runs near-memory-bound (≈85% HBM, near-zero compute waste, near-zero launch overhead). That is the core difficulty: the MoE grouped GEMM is **compute-bound on 64× padding**, not memory-bound, so it structurally cannot reach 85% as a CUTLASS tile-GEMM.
...
**The real lever is bs=1 kernel efficiency — the kernels run at ~12% of the PP-serial HBM floor.** Every fp8 projection in the forward uses a **prefill-oriented kernel** (quant → scale-relayout → tile-padded CUTLASS GEMM) on a workload that is M=1 GEMV / small-group — the wrong primitive. Optimization order (largest headroom first):

1. **MoE grouped FP8 GEMM (29 ms, floor 4.7 ms).** `glm52_trtllm_grouped_fp8_launch` runs `groups=256, m_capacity=512` but only ~8 experts are active at bs=1. The CUTLASS fp8 grouped GEMM has a **64-row M-tile minimum** (`csrc/glm52/glm52_deepgemm_grouped.cu:13` `kExpertAlignment=64`), so each active expert computes 64 rows of which 1 is real → **64× compute waste, structurally compute-bound.** **The obvious fix — DeepGEMM's masked decode kernel — is NOT viable: DeepGEMM is JIT-only (NVRTC at runtime, `third_party/DeepGEMM/CMakeLists.txt:1`), which violates the no-Python-at-build constraint; the `glm52_deepgemm_grouped_fp8_launch` stub returns `CUDA_ERROR_NOT_SUPPORTED` for this reason.** **Decision fork (needs the kernel-strategy call):** (a) a custom fp8 grouped-GEMV decode kernel (M=1 native, memory-bound — but hand-written, which the user discouraged); (b) Triton-AOT a fused-MoE decode kernel (the qwen35 precedent — needs build-time Python for glm52); (c) accept DeepGEMM + build-time Python; (d) tune the CUTLASS/TRTLLM runner for a small-M tactic (most reuse-friendly, no Python/hand-write — but the runner's M-tile is not currently exposed). **The TRTLLM tile-GEMM cannot reach the floor as-is.**
2. **MLA fp8 projections (21 ms, floor 2.7 ms).** ~30 tiny kernels/layer — each projection is quant + scale-relayout + GEMM at M=1. Most reuse-friendly lever (no new GEMM algorithm): **cut the kernel count** — fold the activation-scale relayout into the quant kernel's output layout, and fuse the Q / KV projections that share the same `hidden` input. This is launch/overhead reduction, tractable without a custom GEMM.
3. **MoE glue + shared expert (20 ms, shared-floor 0.6 ms).** router / quant / scatter / weighted-silu-quant / combine all run over the M_CAPACITY=512 padded rows; the shared expert is a per-layer dense fp8 MLP (its own 3 quant+relayout+GEMM kernels). Levers: size the glue to the ~8 active rows; fuse; the shared expert benefits from the same projection-fusion as #2.

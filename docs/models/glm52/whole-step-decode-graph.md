...
> **TL;DR:** Execution record of PR5c: the whole per-rank decode step (embed → 78 layers → lm_head → device argmax) is captured into one CUDA graph and replayed every step. Measured on jz-38 8×H200: **200 → 37.5 ms/step from the graph alone** (byte-identical to the PR5b record), then **→ 31.3 ms/step** after switching every m=1 projection to the weight-only fp8 GEMV (PP8-era kernel; numerics change — activation quant removed — re-gated via the #499 oracle harness in the new `--precision gemv` mode) plus device-bounding the 2080-row MoE recv chain by `expert_offsets[n_local]`. 8-way concurrency stays free (31.0 ms/step, ~248 tok/s aggregate). DeepEP cooperative+cluster launches replay fine under capture (kimi precedent held). Remaining gap to the ~22 ms vLLM DP8/EP8 reference: dispatch wait tail (~med 17 µs but heavy skew tail), combine (~42 µs × 75), FlashMLA + residual small-kernel chain — see the nsys breakdown below.
>

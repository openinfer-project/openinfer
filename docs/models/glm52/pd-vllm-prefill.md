...
> **TL;DR:** qwen3 里程碑 1 与 GLM 里程碑 2 都已完成跨引擎 correctness/performance gate；GLM 采用 vLLM TP8 prefill + OpenInfer EP8 decode、D 端有界轮询、vLLM-compatible hash、tail-block save + router first-token forwarding，并严格保持 D 零 prefill。Pegaflow #395 已合入 tail-save/router 合约；但 OpenInfer #657 叠在 #540 上，Qwen3 的 split K/V host segments → contiguous device page restore 仍依赖未合的 Pegaflow #382。因此 `pegaflow-core` 不能只追到 #395，必须等待同时包含 #382 与 #395 的 `master` revision，再按 #382 → #540 → #657 的顺序落地。
>
...
| 4. `RemoteFetch` 等待分支 | ✅ miss 窗口内重查询（busy path 也有 5ms/请求节流）+ 满窗降级 warn + 熔断（连续 3 请求满窗 miss 后新请求跳过等待，任一远端命中重新武装）+ 启动指纹日志（seed/namespace/block_size/NONE_HASH/几何，与 P 侧肉眼比对）；严格 no-prefill 429/500 开关未做（当前 miss 满窗后 scratch 兜底，输出正确、日志可见） |
...
环境与复跑：8×H200 测试环境的内部工作目录保存组件启停、smoke 和 TTFT A/B 脚本，私有路径不写入本文。OpenInfer 分支 `feat/pd-vllm-hash-compat`；Pegaflow 依赖 [#382](https://github.com/novitalabs/pegaflow/pull/382)（Contiguous←split 加载修复）。**测 P/D 命中前必须重启 D 清 prefix cache，否则量到的是 D 本地缓存**（baseline 请求会把 prompt 灌进 D）。

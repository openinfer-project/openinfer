...
> **TL;DR:** qwen3 里程碑 1 与 GLM 里程碑 2 都已完成跨引擎 correctness/performance gate；GLM 采用 vLLM TP8 prefill + OpenInfer EP8 decode、D 端有界轮询、vLLM-compatible hash、tail-block save + router first-token forwarding，并严格保持 D 零 prefill。Pegaflow #395 已合入并成为 `pegaflow-core` pin。在本项目支持的 normal 配置中，Qwen3 P 固定 vLLM 0.23.0 默认 NHD，GLM P 固定 vLLM 0.24.0；两者写出的 block-local bytes 都与 OpenInfer 对齐，因此 #382 不是依赖。#657 的剩余落地约束只有先合其下层 #540。
>
...
P 存的块能被 D 加载，当且仅当下面四条全部成立。前两条决定 query 是否命中，第三条是 pegaflow 强制的 load 守卫；第四条不在元数据中表达，错了会**静默数据错位**，必须由版本约束和字节 gate 保证：
...
4. **slot 内 K/V 段布局**。Qwen3 的 normal P 配置固定 vLLM 0.23.0 默认 NHD：FA shape 是 `(blocks, 2, block_size, heads, dim)`，connector 注册 `segments=1`，每个 block 的 copy unit 正好是 `[K(block, heads, dim)|V(block, heads, dim)]`，与 OpenInfer 的 page-first fused page 一致。GLM 的 normal P 配置固定 vLLM 0.24.0，MLA/indexer payload 也是已通过字节 gate 的单 segment。两条支持路径都不需要 pegaflow #382；其他 vLLM 版本或非默认 KV layout 不在本 PR 的兼容范围。
...
- **GLM scheduler 与 P/D 的耦合方式**：已在 #657 落为 admission 前完整 restore、`suffix == 1` 才放行，见 §5 与 `pd-m2-execution.md`。

## 5. 完成路径：qwen3 smoke test 先行，GLM 后接

**里程碑 1：qwen3 的 vLLM-P + openinfer-D P/D smoke test —— 已在 8×H200 环境跑通（2026-07）。**
...
| 3. 尾块 connector 扩展 + router 变体 A | Qwen3 #540 不需要，D 最多补 16 token；GLM 已由 Pegaflow #395 + OpenInfer #657 完成，D 以 `suffix == 1` admission |
| 4. `RemoteFetch` 等待分支 | ✅ Qwen3：miss 窗口内重查询 + warn + 熔断，窗口后允许 scratch；GLM #657：完整前缀等待 + 严格 no-prefill reject |
...
环境与复跑：8×H200 测试环境的内部工作目录保存组件启停、smoke 和 TTFT A/B 脚本，私有路径不写入本文。OpenInfer 分支 `feat/pd-vllm-hash-compat`；当前 Pegaflow pin 是合入 [#395](https://github.com/novitalabs/pegaflow/pull/395) 的 `1473c535`。Qwen3 P 固定 vLLM 0.23.0 默认 NHD；GLM P 固定 vLLM 0.24.0。**测 P/D 命中前必须重启 D 清 prefix cache，否则量到的是 D 本地缓存**（baseline 请求会把 prompt 灌进 D）。

**里程碑 2：GLM5.2 接入 —— 已由 #395/#657 完成。** 双 cache（656B MLA + indexer）注册与字节 gate、完整前缀等待、尾页 restore、first-token forwarding 和 strict no-prefill 均已通过；执行记录与实测 gate 见 `pd-m2-execution.md`。

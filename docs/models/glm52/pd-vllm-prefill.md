...
> **TL;DR:** 两个决策已定：(1) **GLM prefill 不自建，走 vLLM（TP8+EP8）**——openinfer glm52 的全部 kernel 面是 decode-only（FlashMLA sparse AOT 写死 `kSq=1`、DeepGEMM MQA `next_n≤2`、DeepEP shim 只有 decode dispatch/combine），且 attention 无任何并行（DP1 rank-0 spine），自建 prefill ≈ 再来一轮 PR1–PR4 规模的 campaign；(2) **P→D 就绪信号用 D 端有界快轮询**——复用 M2 的 `RemoteFetch` 状态机加一个"expect remote"等待分支，不改 vLLM、不做 router callback、不上 RDMA doorbell。TTFT 开销预算 <50ms。最前置的风险项是**跨引擎 hash key 不兼容**（vLLM xxh3_128/CBOR/`PYTHONHASHSEED` 链根 vs kvbm xxh3_64/seed 1337）——决策：openinfer-D 侧做 vllm-hash-compat provider（钉 `xxhash_cbor` + 统一 `PYTHONHASHSEED`），key 体系不另起炉灶；先做字节级兼容 oracle gate。**D 零 prefill 红线**引出尾块问题（vLLM 只 hash 满页）：connector save 路径加一个 P/D-gate 的尾块扩展 + router 把 t1 直发客户端并追加进 D 上下文，D 首步即真 decode。**路线：先在 qwen3 上跑通 vLLM-P + openinfer-D smoke test（通用机制 + D 有 prefill 兜底的安全调试环境），GLM 接入为里程碑 2（依赖 PR5）。**
>
...
**里程碑 1（先做）：qwen3 的 vLLM-P + openinfer-D P/D smoke test。** 所有通用机制在 qwen3 上验证——它有 M2 现成的 D 端机制，且 D 有 prefill 兜底，调试期不会硬失败：

1. **hash + 字节布局兼容 gate**（§3，qwen3 版）——vLLM 的 per-layer K/V 布局 vs openinfer 的 page-first fused buffer，逐字节比对；prompt 两档（对齐/非对齐）。最前置，证伪成本最低。
2. openinfer vllm-hash-compat key provider（canonical CBOR + xxh3_128 + NONE_HASH 推导，golden vector 单测），含尾块 key 推导。
3. pegaflow connector 尾块 save 扩展（P/D 模式 gate，加法式）+ router 变体 A（t1 直发客户端 + 追加进 D 上下文）。
4. openinfer D 端 `RemoteFetch` 等待分支 + "expect remote" 标记 + 严格 no-prefill 模式开关（qwen3 上以配置开关形态验证 429/500 语义，杀 P 做故障注入）。
5. e2e smoke：router + vLLM-P + openinfer-D，greedy 逐字节 vs 直连 baseline，多轮 delta 拉取复验。

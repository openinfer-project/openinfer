...
Semantics preserved from the bs=1 coordinator: EOS suppressed but counted, EOS outranks the length cap, no-logprobs/no-echo/no-LoRA rejections, `prompt + max_tokens − 1 ≤ max_model_len` cap (decided at launch: VRAM-derived per rank, `--max-model-len` overrides), fatal teardown on any step error (a failed step permanently desyncs the EP8 collective group — see `fail_step`). Non-greedy sampling (temperature/top-k/top-p/min_p; per-request seed is plumbed through `mix_seed` but no frontend passes `seed` yet) is supported on the plain decode path since 2026-07: sampling params are range-checked at intake (a bad value past intake would fail the step and tear the engine down); each non-greedy request's committed row takes a post-graph FlashInfer sampling pass over the step logits (`Glm52RankModel::sample_rows_into`, Kimi's own-greedy-path pattern from `openinfer-sample`) instead of the fused argmax; the launch-ahead lease is withheld while any non-greedy request is active (the speculation feeds the argmax token). With the DSpark drafter loaded the engine stays greedy-only (`accept_greedy` verify; rejection sampling is future work). New behavior: a client disconnect frees the rank (send failure during decode, `TokenSink::is_closed` probe during prefill — the bs=1 coordinator decoded to completion into a dead channel). On a failed step the coordinator joins ALL ranks and logs every rank's error before tearing down — the first rank to answer often reports the ~100 s DeepEP device-timeout trap, not the root cause (toxic-review finding).
...
- Sampling e2e (jz-38 8×H200, 2026-07-06): greedy ×2 byte-identical and equal to the bring-up record ("Paris. Distance from Paris to Lyon is 391 km"); `temperature 0.8, top_p 0.95` ×2 fluent and distinct; malformed `top_p ∈ {0, 1.5}` rejected at intake (log: `rejected: GLM5.2 sampling requires top_p in (0, 1]`; surfaces as the known generic 500, #294) with the engine serving on; 8-way concurrent mixed greedy+sampled — 4 greedy identical, 4 sampled distinct.
- GSM8K full (jz-38 8×H200, 2026-07-06, post-rebase on the VRAM-derived max_model_len=72576): lm-eval `local-completions`, 5-shot greedy, `num_concurrent=64` (= the full 8-rank × 8-slot fleet), `max_gen_toks=4096` — **strict-match 94.92% ± 0.60, flexible-extract 94.84%**, 1319/1319 in 13 min 16 s, zero step errors/rejects in the engine log. Stop strings truncate + abort server-side, so answers don't run to the 4k cap. No running-occupancy visibility during the run — follow-up #587.

...
1. **Target-only handoff + explicit draft cold-start.** Transfer the existing 99 target arenas, locally prefill the uncached suffix, and start DSpark with only the aux-hidden context captured for that suffix. With today's sealed-block protocol the suffix is 1–64 tokens; it becomes final-token-only only if a later protocol transfers the partial tail block too. Never mark absent draft pages as valid. Target verification preserves output correctness, but acceptance is unknown. The current OpenInfer drafter encodes `anchor_pos == committed_len + pending_len`; suffix-only startup therefore needs a separate absolute-position base (or equivalent compact-KV mapping), not a fabricated `committed_len` over empty pages.
...
- pegaflow v0.22.6 **lacks the `*_inproc` and `_strided` APIs** this integration uses; inspect the pinned v0.23.2 revision `d46fd16` instead.
...
Feed this audit into the single P/D + NIXL design issue required by `roadmap-2026-h2.md`: define target hash/layout compatibility, sealed-tail semantics, and speculative-state ownership together. Implementation remains **Later** until that design is settled. Its first empirical gate is a #590 suffix-only DSpark A/B after the current 1–64-token local tail prefill; compare that acceptance with the 2.52× stateful payload before selecting draft-KV transfer.

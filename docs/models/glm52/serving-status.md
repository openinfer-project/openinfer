...
1. **pegaflow M1: host-tier KV offload** — implemented on `feat/kv-offload-shared-host` behind `--kv-offload` (shared host pool, 8 rank instances, one namespace, hugepage option); landing gated on the jz-38 evict-restore parity + mixed-load run. Design + implementation notes in `pegaflow-offload-pd.md`.

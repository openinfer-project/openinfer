...
- **Model crate:** every scratch struct sized by the batch; absorb/back GEMMs and lm_head ride the cuBLAS n dimension (the col-major `[N, T]` output IS the row-major `[T, N]` layout the next op consumes); per-row rope rows come from an `embedding_rows` gather over the resident rope table; slot b's cache tokens live at `[b*4096, (b+1)*4096)` with a static block table.

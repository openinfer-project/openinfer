...
- **Plan**:
  ...
  2. Audit the Pegaflow revision range and retain the current pin until a merged `master` revision contains both #382 and #395; update stale dependency documentation now.
...
- **Risks / open questions**:
  - Updating to #395 alone would omit #382 and make the stacked Qwen3 restore path memory-unsafe; the dependency must move only after both changes coexist on Pegaflow `master`.
...
### Unexpected: Pegaflow dependency is a two-PR chain

- Pegaflow #395 merged as `1473c5355d879b4fea23101760cb2a0074642ada`, but that `master` revision does **not** contain still-open #382.
- GLM's page-first single-segment restore does not need #382. The stacked Qwen3 #540 path does: vLLM seals split K/V host segments while OpenInfer restores into a contiguous `[K|V]` device page. Without #382, the V restore silently reads past the K segment.
- Decision: do not re-pin to #395 alone. The final OpenInfer pin must name a Pegaflow `master` revision containing both #382 and #395; until #382 merges, #657 remains dependency-blocked even if its local CI is green.
...
- **Dependency decision:** keep `pegaflow-core` at the #381 revision. Pegaflow #395 supplies the GLM tail-save/router contract, but #657 is stacked on #540 and therefore also needs #382's split-host-to-contiguous-device restore. Advancing to #395 alone would make green CI a false safety signal.
- **Landing order:** merge Pegaflow #382, select a Pegaflow `master` revision containing #382 and #395, update and verify the OpenInfer pin, land #540, then remove the stacked #540 commits from #657.
- **Remaining validation:** this maintenance pass does not replace the already recorded two-node performance/correctness gates. Re-run the cross-engine smoke after the final Pegaflow revision is available because the dependency boundary changes the restore implementation.
...
- 依赖悬挂：OpenInfer #540 与 Pegaflow #382 都未合并；GLM 分支叠在 #540 上，当前 rev pin 保持 #381，直到 Pegaflow `master` 同时包含 #382 与已合并的 #395。

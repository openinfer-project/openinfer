...
> **TL;DR:** `openinfer-glm52/benches/` 的 Blackwell decode 逐内核调优台：**Python harness**（torch 参考 + 一键 check/bench/compare）调用 build.rs 用生产 flags 编出的 `libglm52_kernel_lab.so`（内核源码仍是 `csrc/glm52/*.cu` 单一事实源），bench 单位画在**生产融合内核边界**上。19 个 EP 无关 per-rank 单元在册（bucket/ctx 轴，单卡可测、无需 NCCL）；MoE 专家链（原 moe_ep_wo 组）随 #798 DeepGEMM 替换退役，重建到 `glm52_deepgemm_grouped_sm100.cu` 为待办。gemv 多子块 mma（rows 16/32/64，GB300 实测表）由 `KERNEL_LAB_GEMV_MMA_MULTI` 门控、与生产 #812 register tile 的 A/B 待跑。目标是人和 agent 一条命令完成"改内核 → 对拍 → 压测 → 对比基线"，顶门是 `glm52_step_bench` 同会话 A/B。
>
...
- **代码事实**（explore agent 核实）:
  ...
  - weights-free 自验证骨架：`openinfer-kernels/tests/glm52_moe_ep_wo_smoke.rs`（EP4/EP8/EP64 shape 已参数化、host f32 参考、Lcg RNG）、`glm52_sparse_mla.rs`（f64 naive 参考）、`e4m3_to_f32` helper（2026-08 注：两个 smoke 骨架已随 #797/#798 删除，见 Execution Log 2026-08-01）
  - decode 路径：`run_step_body`（`openinfer-glm52/src/model/step_body.rs:47`）；EP 链选择 `Glm52MoeEpState`（`moe_ep_wo.rs:300`）；拓扑枚举 `Glm52MoeTopo`（`lib.rs:202`，EP8→32 experts/rank、EP4→64、EP16→16）（2026-08 注：moe_ep_wo 链已退役为 DeepGEMM）
  - Blackwell 上 EP 任意宽度只有 `glm52_moe_ep_wo` 链可 bench（DeepGEMM masked 是 sm_90a-only）；注意力后端 = FlashMLA sparse（sm_100f）（2026-08 注：DeepGEMM 已移 sm_100，ep_wo 链删除）
...
- 2026-08-01 rebase 到 main@8dcddde（42 个新提交之后的冲突实况与处置）:
  - **moe_ep_wo 单元组退役**：main #797/#798 删除 `glm52_moe_ep_wo.cu` 整文件（routed
    expert 改走 SM100 DeepGEMM masked grouped，HO/权重链一并移除）。分支 42d43e1 的
    masked_mma kPipe 调优随之作废（其引擎级 -4.3% A/B 数字记录在 07-29 条目中，仅作
    历史）。本侧删除 `moe_ep_wo.{tiles,w13_mma,silu,w2_mma}` 的 refs/adapter/manifest/
    test/baseline；其余 19 单元的 FFI 符号逐一和 main csrc 核对存活。**待办：二期 MoE
    单元重建到 `glm52_deepgemm_grouped_sm100.cu`（masked grouped 的新契约）。**
  - **gemv 多子块 mma 与 #812 verify-span register tile 的竞争合并**：main 在 16/32/48
    加了"正确优先"register tile（生产 decode 实际 rows>8 走 fp8 GEMM，
    FP8_GEMV_MAX_ROWS=8）；本分支有 GB300 实测 mma 表（{16,32,64}）。合并语义 =
    `KERNEL_LAB_GEMV_MMA_MULTI` env 门控：默认关、生产路由与 main 逐字节一致；
    harness 内 loader.py 默认置 1（否则 rows=64 无路径）。**待办：(a) rows 16/32 上
    mma vs register tile 的 A/B（knob 0/1）；(b) batch=48（bucket-6×span-8 满占）mma
    表首扫；(c) 依 A/B 决定生产 16/32 是否切表。**
  - build.rs kernel-lab .so 块无冲突重放；CPU pytest 88 绿（重跑于 rebase 后工作树）；
    `GLM52_GEMV_MMA_SCRATCH_FLOATS_PER_ROW=16·24576` 复核仍覆盖多子块表
    （16·16384，o_proj rows=64 最大）。
  - 分支从 7 提交 squash 为 2 提交（harness 基建 + gemv mma）；旧指针存
    backup/glm52-kernel-lab-pre-rebase。

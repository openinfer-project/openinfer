...
checked on 2026-06-15. On fixed-length single-concurrency probes, openinfer
leads first-token latency: `101.8 ms` vs `115.2 ms` TTFT p50 on 2048/1 and
`53.7 ms` vs `67.4 ms` on 1024/256. vLLM keeps the steady decode edge:
`6.32 ms` TPOT p50 vs openinfer `7.31 ms`, or `152.3` vs `133.6` output tok/s.
...
| 2048 input / 1 output | TTFT p50 | **101.77 ms** | 115.23 ms | openinfer 11.7% lower |
| 2048 input / 1 output | TTFT p99 | **108.69 ms** | 123.73 ms | openinfer 12.1% lower |
| 2048 input / 1 output | request/output tok/s | **9.94** | 8.61 | openinfer 15.5% higher |
| 1024 input / 256 output | completed | 30/30 | 30/30 | both clean |
| 1024 input / 256 output | TTFT p50 | **53.75 ms** | 67.38 ms | openinfer 20.2% lower |
...
- The table uses the client-requested input/output lengths as the workload
  ...
  servers on these random text prompts, so the comparison should be read as a
  `vllm bench serve` client-contract result rather than a token-id replay.
...
- vLLM still has the decode TPOT edge on this shape. The openinfer result to
  carry into the README is narrower: lower first-token latency on both fixed
  shapes, with a decode throughput gap still visible.

...
- Built OpenInfer server binary in the validation worktree:
  - `OPENINFER_CUDA_SM=120 OPENINFER_TRITON_PYTHON=<python-venv>/bin/python cargo build --release -p openinfer --bin openinfer`
...
- OpenInfer served model ID was `<model-path>`, not `Qwen3-4B`, so the client model name was set to `<model-path>`.
- Measured JSON:
  - `openinfer-in4096-out64-c1-n20.json`
- Command shape:
  - server: `target/release/openinfer --model-path <model-path> --port 8000`

...
| `models/glm52/dp8-scheduler.md` | PR5b: DP8 lock-step scheduler — non-expert stack replicated to every rank, one request per rank, every step all 8 ranks forward one token (real or padding) into the DeepEP collectives (`GLM52_DECODE_GLOBAL_TOKENS` 1→8, DP1 = active_ranks=1 special case). Pure `Glm52SlotState` state machine + thin coordinator shell. jz-38 e2e byte-parity gate pending. |

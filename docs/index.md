...
| `models/kimi-k2/pplx_marlin_compute_report.md` | PPLX routed local compute H20 report：synthetic stress provider 与 trace-driven replay provider 均已覆盖 routing/W13/SwiGLU/W2；p95 NCU 显示 W13/W2 是 single-wave/shared-memory-limited，不是 HBM 或 tensor core 极限。 |
| `models/kimi-k2/tp1-pplx-decode-bench.md` | Implemented `kimi_tp1_pplx_decode_bench` and `kimi_pplx_marlin_replay`: TP1/PPLX operator bench with roofline fields, NCU filters, synthetic PPLX providers, runtime route histogram trace, trace-driven local Marlin replay, and p95 NCU isolation. |

...
| `models/glm52/tp4-prefill-only.md` | Native eager TP4 prefill-only serving on one 4-GPU host: shared 16K coordinator chunks, 32-row reused-kernel tiles, multiple requests, required prefix cache, one-token output, no decode, and VRAM-derived context capacity. |
...
| `models/glm52/native-mtp-accuracy.md` | Native MTP5 passes a production-path acceptance check: the first five-token proposal is exact, four comparable rounds accept `[1,1,1,1]` drafts versus official vLLM's `[1,1,1,2]`, and the target token stream stays aligned. Aggregate acceptance remains lower on OpenInfer's different target topology, while measured GSM8K quality is stable. |

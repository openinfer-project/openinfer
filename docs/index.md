...
| `models/qwen3/dflash-speculative-decoding.md` | Qwen3-4B DFlash is wired for greedy TP1 serving with native drafter/verifier/scheduler, INFO acceptance logs with `committed_tokens`, profiling-only timing, verifier-span/full-draft HF regret gate, request-local draft scratch, reusable pending-context buffer, config-derived DFlash memory reserve, DFlash-aware context admission, chunked-prefill continuity checks, and DFlash small-N cublasLt tuning. 5070 Ti Step 9 Spec-Bench bs=1 is 148.34 tok/s; 5090 OpenInfer Spec-Bench bs=1 is 251.48 tok/s; vLLM 0.22.1 DFlash supports the same draft and reaches 289.57 tok/s with native acceptance metrics. |

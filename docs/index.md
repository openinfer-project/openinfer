...
| `models/glm52/cross-node-scaling.md` | Design doc (no code, supersedes dp16-smr-coordinator): DP16 = 2×8 H200 in three planes. Data plane = DeepEP v2 GIN scale-out (shim already builds GIN contexts; un-bake four single-node spots, re-derive per-scale config) — two-node dispatch/combine microbench is the go/no-go gate. Control plane = framed-TCP hub-and-spoke: one global coordinator, dumb rank-host, one conn/node with FIFO `Response` + push `Event` frames, fail-stop, no framework. Facts plane = node-local async facts (offload saves, P/D KV pulls) push as Events; ≤1-step-stale mirror ≡ local poll under lock-step. DP32/64: EPLB becomes prerequisite, blast radius × N, DP-2N must beat 2×DP-N measured; >4 nodes or single-digit-ms steps upgrades control to the preserved SMR/Calvin design (replicated deterministic coordinator + sequenced streams + free replay journal). |

...
| `models/kimi-k2/profiles/README.md` | Kimi-K2 TP1/DP8/EP8 decode profiling 入口：master ledger、TP1 PPLX bench log、fusion scan 和每个 `*_report.md` 都收在 `profiles/` 下。 |
| `models/kimi-k2/profiles/tp1-dp8-ep8-decode-optimization-master.md` | Kimi-K2 TP1 DP8 EP8 decode 优化 master ledger：H20 per-rank bs=8/global bs≈64 全 decode 算子表、roofline 分类、peak gap、fusion 扫描队列和单 kernel report 队列；FlashInfer MLA decode planned `partition_kv` bench probe 在 synthetic ctx4096/8192 和 fixed-grid production-cap ctx1024/2048 都有近 2x 信号且 equivalence 通过，但 production PPLX adoption 因 global bs64 runtime gate 失败而拒绝，未采纳的 probe kernel 已从代码清理；PPLX Marlin local compute 使用 runtime route-hist replay p95 baseline。 |
| `models/kimi-k2/profiles/tp1-decode-blog.md` | Kimi-K2 TP1+DP8+EP8 decode 逐算子**理论**性能剖析（博客）：按执行顺序覆盖一个 decode step 的全部 28 个算子 + 两段 EP all-to-all，从计算特性角度分析 FLOPs/字节/AI/roofline，结论用变量表达（GEMM `AI=B`、INT4 `AI=4·M_e`、MLA 仅 §9 随 ctx）。开头有写作者风格指南（理论口径、禁实现细节/优化结论/主观词，B 为自由变量）。叙事层，数据以 master ledger 为准。 |
| `models/kimi-k2/profiles/tp1-dp8-ep8-fusion-scan.md` | Phase 2 fusion scan for TP1 DP8 EP8 decode 已完成：H20 NCU 覆盖 `shared_gate_up -> shared_swiglu`、`attention input_norm -> qkv_a`、`qkv_a_split_norm -> q_b`；qkv_a cuBLASLt 和 stock CUTLASS gated-dual GEMM 均已拒绝，无 accepted fusion，剩余方向转 Phase 3 custom/single-kernel work。 |
| `models/kimi-k2/profiles/embedding_report.md` | `decode.embedding` H20 report：TP1 vocab-sharded embedding lookup 为 control/lookup，`6.83-7.24us`、`224` CTAs、`31.7-33.6GB/s` payload，停止 standalone 调参。 |
| `models/kimi-k2/profiles/dense_gate_up_report.md` | `decode.dense.gate_up` H20 report：dense layer0 gate/up BF16 GEMM 为 memory-bound，`147.96us` / `3.58TB/s` / `74.5%` H20 HBM；单层单次调用，停止 standalone 调参。 |
| `models/kimi-k2/profiles/dense_swiglu_report.md` | `decode.dense.swiglu` H20 report：dense layer0 SwiGLU 为 control/elementwise，`7.79us`、`576` CTAs、`113.6GB/s` payload，停止 standalone 调参。 |
| `models/kimi-k2/profiles/dense_down_report.md` | `decode.dense.down` H20 report：dense layer0 down BF16 GEMM 为 memory-bound，`85.48us` / `3.10TB/s` / `64.5%` H20 HBM；单层单次调用，停止 standalone 调参。 |
| `models/kimi-k2/profiles/dense_residual_add_report.md` | `decode.dense.residual_add` H20 report：dense layer0 residual add 为 control/elementwise，`6.81-7.51us`、`224` CTAs、`45.8-50.5GB/s` payload，停止 standalone 调参。 |
| `models/kimi-k2/profiles/pplx_residual_add_scaled_report.md` | `decode.moe.residual_add_scaled` H20 report：PPLX post-combine scaled residual add 为 control/elementwise；NCU 确认 `224` CTAs、`0.36` waves/SM、`8.37%` SM、`3.33%` DRAM、`91.25%` no-eligible，停止 standalone 调参。 |
| `models/kimi-k2/profiles/kimi_router_noaux_tc_report.md` | `decode.moe.router` H20 report：control/small-grid limited；保留 pedantic logits GEMM，采纳 post-GEMM score/topk fusion，TP1 PPLX `bs=8,ctx=1` 从 `3.655ms` 到 `3.514ms`；fast tensor-op logits GEMM 因 TP1 DP8 bs64/o5 token trace `30/64` mismatch 仍拒绝。 |
| `models/kimi-k2/profiles/shared_gate_up_report.md` | `decode.moe.shared_gate_up` H20 report：memory-bound BF16 skinny GEMM，采用 Kimi 专用 cuBLASLt exact-shape path，TP1 PPLX `bs=8,ctx=1` 从 `1.818ms` 到 `1.505ms` per 60 MoE layers。 |
| `models/kimi-k2/profiles/shared_swiglu_report.md` | `decode.moe.shared_swiglu` H20 report：standalone shared SwiGLU 是 tiny-grid/latency limited（`64` CTAs、`0.10` waves/SM、`0.51%` DRAM read），停止单独优化，只保留 row21/22 或 row22/23 custom fusion 方向。 |
| `models/kimi-k2/profiles/shared_down_report.md` | `decode.moe.shared_down` H20 report：memory-bound BF16 skinny GEMM，TP1 PPLX `bs=8,ctx=1` 当前 `897.1us` per 60 MoE layers；standalone cuBLASLt `11.000us -> 10.995us` 无有效收益，后续只看 row 22/23 fusion。 |
| `models/kimi-k2/profiles/attention_o_proj_report.md` | `decode.attention.o_proj` H20 report：memory-bound BF16 skinny GEMM，采用 Kimi TP1 cuBLASLt exact-shape path，TP1 PPLX `bs=8,ctx=1` 从 `2.715ms` 到 `2.374ms` per 61 attention layers。 |
| `models/kimi-k2/profiles/qkv_a_proj_report.md` | `decode.attention.qkv_a` H20 report：memory-bound BF16 skinny GEMM，TP1 PPLX `bs=8,ctx=1` 当前 `1.245ms` per 61 attention layers；standalone cuBLASLt 只有 `0.8-1.7%`，后续只看 row 6/7 RMSNorm-prologue fusion。 |
| `models/kimi-k2/profiles/attention_absorb_q_nope_report.md` | `decode.attention.absorb_q_nope` H20 report：TP1 MLA strided-batched BF16 GEMM，采用 cuBLASLt path，TP1 PPLX `bs=8,ctx=1` 从 `973.6us` 到 `748.5us` per 61 attention layers。 |
| `models/kimi-k2/profiles/attention_v_up_report.md` | `decode.attention.v_up` H20 report：TP1 MLA strided-batched BF16 GEMM，采用 cuBLASLt path，TP1 PPLX `bs=8,ctx=1` 从 `781.0us` 到 `738.5us` per 61 attention layers。 |
| `models/kimi-k2/profiles/q_b_proj_report.md` | `decode.attention.q_b` H20 report：memory-bound skinny BF16 GEMM，cuBLASLt exact-shape sweep 在目标 `batch_size=8` 只有 `8.899us -> 8.746us` (`1.0175x`)，已拒绝 standalone 替换；后续只看 row 8/9 fusion。 |
| `models/kimi-k2/profiles/attention_input_norm_report.md` | `decode.attention.input_norm` H20 report：FlashInfer RMSNorm 是 tiny-grid/launch limited（`8` CTAs、`0.05` waves/SM、`0.70-0.74%` DRAM），停止 standalone RMSNorm，后续只看 RMSNorm → qkv_a prologue/custom GEMM。 |
| `models/kimi-k2/profiles/attention_qkv_a_split_norm_report.md` | `decode.attention.qkv_a_split_norm` H20 report：Kimi split/norm helper 是 tiny-grid/launch limited（`8` CTAs、`0.01` waves/SM、`0.19-0.20%` DRAM），停止 standalone row8，后续只看 row8 → q_b prologue/custom GEMM。 |
| `models/kimi-k2/profiles/attention_rope_split_report.md` | `decode.attention.rope_split` H20 report：Kimi RoPE split helper 是 control/elementwise；NCU 确认 `384` CTAs、`0.62` waves/SM、`10.51%` SM、`1.27%` DRAM、`77.03%` no-eligible，停止 standalone 调参。 |
| `models/kimi-k2/profiles/attention_paged_kv_append_report.md` | `decode.attention.paged_kv_append` report：provider 已改用 production page metadata 并标为 control；H20 `ctx=1` 为 `7.07us/call`，production NCU 确认 `78` CTAs、`0.12` waves/SM、`1.50%` SM、`0.09%` DRAM、`97.63%` no-eligible，停止 standalone 调参。 |
| `models/kimi-k2/profiles/attention_flashinfer_mla_decode_report.md` | `decode.attention.flashinfer_mla_decode` H20 report：ctx-sensitive；selected NCU 显示 baseline 低 grid；bench-only planned `partition_kv` p128 把 synthetic ctx4096/8192 降到 `26.7/52.5ms`，fixed-grid p32 把 production-cap ctx1024/2048 降到 `7.31/13.85ms`，但 production PPLX wiring 因 global bs64 runtime gate 失败而拒绝，未采纳 kernel 已清理。 |
| `models/kimi-k2/profiles/attention_post_attn_add_norm_report.md` | `decode.attention.post_attn_add_norm` H20 report：exact-preserving fused add + RMSNorm round 是 tiny-grid/control；NCU 确认 `8` CTAs、`0.05` waves/SM、`2.29%` SM、`1.11%` DRAM，停止 standalone 调参。 |
| `models/kimi-k2/profiles/final_norm_report.md` | `decode.final.norm` H20 report：最终 FlashInfer RMSNorm 与 attention input norm 同 shape，`8.01us/call`、`57.3GB/s` payload；同 shape NCU 为 `8` CTAs / `0.05` waves/SM / `<1%` DRAM，停止 standalone 调参。 |
| `models/kimi-k2/profiles/final_lm_head_report.md` | `decode.final.lm_head` H20 report：BF16 full-vocab GEMM 已到 `542.7us` / `4.33TB/s` / `90.3%` H20 HBM，停止 standalone 优化，后续只看量化/FP8 格式级变化。 |
| `models/kimi-k2/profiles/final_argmax_report.md` | `decode.final.argmax` H20 report：把 one-CTA-per-row BF16 top1 改成 split-vocab partial reduction，TP1 PPLX `bs=8,ctx=1` 从 `125.3us` 到 `12.7us`，TP1 DP8 bs64/o5 token A/B `0/64` mismatch。 |
| `models/kimi-k2/profiles/pplx_build_marlin_routing_report.md` | `decode.moe.pplx_build_marlin_routing` H20 report：PPLX Marlin routing metadata builder 为 one-block control row，trace replay p95 `9.87us/call`、`592.3us/step`，NCU `1 x 64` / `0.04%` DRAM，停止 standalone 调参。 |
| `models/kimi-k2/profiles/pplx_marlin_compute_report.md` | PPLX routed local compute H20 report：synthetic stress provider 与 trace-driven replay provider 均已覆盖 routing/W13/SwiGLU/W2；已采纳 small-N Marlin tile，p95 W13/W2 `250.64/138.51us -> 161.45/98.14us`，后续只看 route-aware grouped/persistent 设计。 |
| `models/kimi-k2/profiles/pplx_swiglu_report.md` | `decode.moe.pplx_swiglu` H20 report：PPLX routed SwiGLU 为 compute/elementwise，trace replay p95 `12.66us/call`、`759.7us/step`，NCU `6784 x 256` / `55.4%` SM / `6.3%` DRAM，停止 standalone 调参。 |
| `models/kimi-k2/profiles/tp1-pplx-decode-bench.md` | Implemented `kimi_tp1_pplx_decode_bench` and `kimi_pplx_marlin_replay`: TP1/PPLX operator bench with roofline fields, NCU filters, production-metadata MLA KV append provider, trace-driven local Marlin replay, p95 NCU isolation, and a rejected FlashInfer MLA `partition_kv` probe whose unused kernel code has been removed. |

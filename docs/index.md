...
| `models/glm52/pd-m2-execution.md` | GLM P/D M2 gates are complete: cross-engine token parity, strict zero-prefill, failure recovery, equal-card A/B, gsm8k parity, and NIAH 36/36. #657 is refreshed against current main; landing remains ordered behind Pegaflow #382 and stacked OpenInfer #540. |
| `models/glm52/pegaflow-offload-pd.md` | GLM5.2 target-only P/D uses 99 target arenas plus vLLM hash/layout compatibility. The pegaflow-core pin intentionally stays at rev d46fd16 until one merged master revision contains both #382 (needed by stacked Qwen3 #540) and #395 (GLM tail-save/router). |
| `models/glm52/serving-status.md` | Living status: decode serving feature-complete for its scope; target-only cross-engine P/D is implemented and gated in #657 but lands after Pegaflow #382 and stacked OpenInfer #540. DSpark state transfer remains separate scope. |
...
| `models/glm52/pd-vllm-prefill.md` | Decision record and completed gates for vLLM prefill → OpenInfer decode. Pegaflow #395 merged the tail-save/router contract; #657 remains stacked on #540 and therefore cannot update its dependency safely until Pegaflow #382 also lands. |

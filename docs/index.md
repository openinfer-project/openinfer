...
| `models/glm52/whole-step-decode-graph.md` | PR5c design + execution: capture the whole per-rank decode step (embed→argmax) into one CUDA graph replayed every step, targeting the ~200 ms fixed step cost. Full blocker inventory with file:line; three gated stages — persistent arena, host-quiet step (indexer D2H fold kernel, device cache-pack slot, device top-1), capture via `CudaGraphState`. DeepEP cooperative launches under capture retired by kimi precedent. |

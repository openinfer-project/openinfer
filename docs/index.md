...
| `projects/deepseek-moe-tilelang-review.md` | Active: official TileKernels MoE maps routing/layout, not a drop-in expert MLP; persistent rank workers plus decode-only direct top-k MoE cut 1x32 steady TPOT to `80.49ms/token`; NVTX+CUDA trace proves the remaining nsys/Amdahl bucket is rank arrival skew before `107` f32 collectives/token, not raw NCCL transfer. |

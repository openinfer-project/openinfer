...
| Path | TL;DR |
| --- | --- |
| `models/glm52/moe-tp8-low-latency.md` | Plan: replace bucket-1 decode MoE EP8 chain (~103–145 µs/layer, dispatch is rank-arrival-wait bound) with one TP8-sharded cooperative persistent kernel + LL packet reduce-scatter per layer (~36 µs/layer in a same-node prototype); target solo 19.5 → ~14–15 ms/step. Sharding becomes a launch-time config — EP8 stays as the high-throughput layout. P0 mechanism probes (peer access × NCCL windows, cooperative launch under capture) gate everything. |

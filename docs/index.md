...
| Path | TL;DR |
| --- | --- |
| `models/qwen3/serving-perf-5090.md` | Qwen3-4B vs vLLM 0.22.1 tuning record: batched step tail (#345), chunked prefill (default 2048), bs8/16 bucket removal, **cuBLAS 12.9 N=1025 cliff (build with CUDA ≥ 13)**, cublasLt/split-KV/argmax decode tuning. Beats vLLM at QPS1 on all metrics, TTFT everywhere, and overload outright; remaining gap is mid-band TPOT (unified-step attention split). |

...
| `models/qwen35/prefix-cache.md` | Qwen3.5 prefix-cache design: prefix hits must restore full-attention KV plus recurrent-state snapshots at the same boundary; uses a fixed 256-token snapshot interval, 52 MB snapshots, GPU+CPU two-tier pool, and LRU eviction. |

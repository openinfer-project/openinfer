...
| `models/glm52/ep1-forward.md` | PR3 plan (supersedes the plan's PR3 section): MoE/dense/bookend bricks + decoder-layer composition, EP1 brick-scale with oracle gates — no single-GPU full-model e2e (experts ≈ 700 GiB, e2e moves to PR4). MoE chain shaped to the DeepEP v2 elastic shim contract (expert-major + device psum → TRTLLM grouped GEMM); CUDA-graph capturability is the acceptance bar; cross-layer top-k sharing (21/78 layers run the indexer). |

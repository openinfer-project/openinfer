...
| `models/kimi-k2/pplx_marlin_compute_report.md` | PPLX routed local compute H20 report：synthetic expected-local-route provider 覆盖 routing/W13/SwiGLU/W2；H20 varied-prompt route histogram 显示 active8 padded rows/rank p50/p95/max `80/216/336`，synthetic `400` 偏保守，仍需 trace-driven provider 替换 synthetic latency。 |
| `models/kimi-k2/tp1-pplx-decode-bench.md` | Implemented `kimi_tp1_pplx_decode_bench`: TP1/PPLX operator bench with roofline fields plus `--ops`/`--labels` filters for NCU isolation; current accepted paths、synthetic PPLX Marlin providers、runtime varied-prompt TP1/DP8/PPLX route histogram trace are recorded. |

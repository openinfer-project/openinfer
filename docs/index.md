...
| Path | TL;DR |
| --- | --- |
| `models/qwen35/roadmap.md` | Qwen3.5-4B roadmap (2026-06 review): decode-correct and still improving; current branch improves direct OpenInfer TPOT by 2-3%, but vLLM still leads 1024/256 HTTP decode and high-concurrency throughput. Nsight points the next gap search at serving/scheduler/event sync. |
...
| `models/qwen35/optimization.md` | Hybrid 24 linear + 8 full attn optimization ledger. Latest branch fuses MLP gate/up and tunes decode cublasLt buckets, improving OpenInfer direct TPOT by 2-3%; vLLM 0.23.0 still leads 1024/256 HTTP decode, with serving/scheduler profiling now the next gap target. |

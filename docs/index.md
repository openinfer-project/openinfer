...
| `models/glm52/pd-m2-execution.md` | GLM P/D M2 执行记录（2026-07 启动）：vLLM TP8 prefill + openinfer EP8 decode（无 MTP），跨节点 pegaflow P2P，验收 = D 零 prefill 硬约束 + 多轮长输入压测 vs 2× mixed。GLM 专属项：双 cache 字节 gate、尾块 partial-block restore、路由变体 A、严格 fail 语义。 |

...
| `models/qwen3/roadmap.md` | Qwen3-4B maturity roadmap: TP=2 lifecycle coverage exists but numerical parity remains open; real-adapter LoRA accuracy, prefix-cache observability, and YaRN #8 are the other current qualification gaps. |
...
| `lessons/kv-full-lifetime-admission.md` | Scheduler invariant for paged KV without preemption: reserve each admitted request's worst-case KV writes, defer temporary pressure, explicitly reject impossible requests, clean up every terminal path, and prove recovery with a post-pressure completion. |

...
| Path | TL;DR |
| --- | --- |
| `models/glm52/support.md` | GLM5.2 bring-up: `openinfer-glm52` plus the server/bench `glm52` feature now validates `glm_moe_dsa`, stop tokens, safetensor manifest, all-rank EP8 load plans, rank0 dtype/shape headers, rank-worker raw H2D weight loading, non-expert FP8 weight contract (`666` projections per rank: `432/9/225` attention+dense+shared), streaming FP8 routed-expert packages, GLM-specific DeepEP shape/capacity, persistent full-layer decode arenas, GLM router + DeepEP + DeepGEMM-style grouped-layout metadata + MoE activation-quant including DeepGEMM TileLang-source weighted W2-input SwiGLU quant + DeepGEMM F32 scale-layout + FlashInfer/TRTLLM grouped-offset scale relayout + TRTLLM grouped-FP8 raw-runner workspace gate, and a fail-closed `DP8 TP1 EP8`/DeepEP `EngineHandle` with 1 coordinator + 8 rank workers; jiuzhang node38 ignored IT loads the real checkpoint and validates expert package + arena residency, synthetic all-EP DeepEP decode smoke, real-router top-k normalization + DeepEP route smoke, W13 quant + weighted W2-input quant, DeepGEMM scale-layout smoke, TRTLLM grouped-offset scale relayout smoke, all-layer MoE GEMM contract `3..=77`, TRTLLM W13/W2 `workspace_bytes=0`, real W13/W2 grouped-FP8 decode smoke, fixed-bucket decode CUDA Graph capture/replay with W13/W2 inside the graph, and OpenAI HTTP request-time rejection via clap CLI args. First runnable forward is decode-only for future P/D split: prefilled KV/page state is assumed, prompt prefill is out of scope, decode must be real batched `bs > 1`, MoE uses fixed DeepEP layout and grouped expert GEMM, and full decode CUDA Graph integration is required. Unified mixed batch, MTP, hidden prefill fallbacks, per-token prefill loops, and per-request bs=1 decode loops are prohibited. Kernel ledger searches FlashInfer, DeepGEMM, FlashMLA, DeepEP, `../vllm`, then cuBLASLt; H200 routed experts start from grouped FP8, not SM100 MegaMoE/FP8xFP4, and handwritten/local CUDA kernels need NCU. |
| `models/glm52/decode-forward-contract.md` | Shared GLM5.2 decode-forward tensor contract: fixed `bs=128` bucket, attention/indexer/MLP/MoE/logits shapes, per-layer DAG, current ABI coverage, source-mapped FlashMLA sparse attention/indexer/dense/logits/graph routes, and an implementation split that starts with generic FP8 projection glue plus an MLA layout probe before full graph integration or later PP8 migration. |
| `models/glm52/pp-decode.md` | PP8 decode exploration for GLM5.2 low latency: preserves the TP/PP roofline and jiuzhang H200 P2P handoff data (`12KB` hidden half-RTT proxy `~2.2-2.3us`, 7-stage total `~16us`), defines a first `PP8 TP1 EP1` candidate with stage-local layer slices and all 256 experts per stage layer, and tracks which current GLM52 kernels migrate unchanged versus which PP-specific pieces are still needed (local MoE permute/combine, stage weight packaging, graph-internal P2P send/wait, PP scheduler/runtime). |
| `models/glm52/decode-kernel-map.md` | Per-op kernel reuse map for the GLM5.2 PP8 decode forward (ultracode 7-op survey): 5 of 7 op-families (MLA sparse decode, MoE grouped FP8 GEMM, router noaux_tc, dense/shared FP8 linear, embed/norm/lmhead/sample) are already CUDA-implemented + Rust-wrapped from the dropped DP/EP work; only the DSA indexer top-2048 (port vLLM `persistent_topk`) and interleaved-RoPE wiring need new CUDA; the rest is glue (576 absorbed query, 656-byte V3.2 KV pack) + PP8 orchestration. Two verified correctness notes: router `route_scale=2.5` (routed_scaling_factor, not the 1.0 placeholder) and `rope_interleave=True`. Maps remaining work to build Slices 2-7. |
| `models/glm52/mla-layout-probe.md` | GLM5.2 MLA layout — RESOLVED as bog-standard 64-head DeepSeek-DSA (no head-fold). The earlier "factor-4" theory was a corrupt `GLM-5.2-0614-Provider-FP8` repack (q_b/kv_b 4× oversized, o_proj correct → internally inconsistent); official `zai-org/GLM-5.2-FP8` is `q_b[16384,2048]`/`kv_b[28672,512]`/`o_proj[6144,16384]`, identical to GLM-5.1. Holds the standard decode-forward contract + FlashMLA-sparse reuse pointers; the `mla_layout_probe` IT re-asserts the shapes against `/data/models/GLM-5.2-FP8`. |
| `models/glm52/vllm-kernel-reference.md` | vLLM kernel reference for GLM5.2: direct-copy CUDA/C++ candidates include GLM-5 router GEMM, grouped noaux top-k, MoE unpermute/reduce, indexer cache helpers, sparse-indexer top-k, and some FP8 quant kernels; DeepGEMM MoE and DeepEP are contract-first because much of their glue is Python/Triton/third-party API composition; current GLM52 uses DeepGEMM TileLang `swiglu_apply_weight_to_fp8` semantics for weighted W2-input quant rather than a standalone route-weight multiply. |
| `models/glm52/vllm-moe-fp8-kernels.md` | Focused vLLM MoE/FP8 kernel map for GLM5.2: vLLM's Hopper+EP block-FP8 policy prefers FlashInfer CUTLASS, DeepGEMM provides the strongest layout oracle for current expanded DeepEP work, vLLM CUTLASS 3.x remains the closest raw CUDA/C++ fallback source, and FlashInfer/TRTLLM `GroupedWithOffset` now has GLM's `11232`-row scale substrate plus a fixed W13/W2 raw-runner workspace gate, real W13/W2 launch smoke, and fixed-bucket decode graph evidence; H200 perf/NCU and full-layer integration are next. |
...
| `conventions/coding-style.md` | Testing principle: keep a test only if deleting it would lower correctness confidence; prefer integration tests, don't test what E2E catches. |

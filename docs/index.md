...
| `models/glm52/pd-m2-execution.md` | GLM P/D M2 gates are complete: cross-engine token parity, strict zero-prefill, failure recovery, equal-card A/B, gsm8k parity, and NIAH 36/36. #657 is refreshed against current main and pins merged Pegaflow #395; only the stacked OpenInfer #540 landing order remains. |
| `models/glm52/pegaflow-offload-pd.md` | GLM5.2 target-only P/D uses 99 target arenas plus vLLM hash/layout compatibility. The pegaflow-core pin is v0.23.3 rev 1473c53 (#395); the pinned normal vLLM configurations already match OpenInfer, so #382 is not required. |
| `models/glm52/serving-status.md` | Living status: decode serving feature-complete for its scope; target-only cross-engine P/D is implemented and gated in #657, which remains stacked on OpenInfer #540. DSpark state transfer remains separate scope. |
...
| `models/glm52/ep4-gb300.md` | DP4/EP4 (`--moe-topo ep4`) on 4xGB300: 64 whole experts/rank (188 GiB weights/rank), second DeepEP shim instantiation, and a weight-only routed-expert chain (bf16×fp8 mma on the aligned recv layout — no act re-quant/relayout/remap) replacing the sm_90a DeepGEMM masked GEMMs. Bring-up green 2026-07-12 incl. layer-6 EP4 oracle 63/64 ×4 buckets (needed the FlashMLA sm100 UE8M0 KV-scale fix — see lessons); c=32 `559 tok/s` / TPOT p50 `39.05ms` untuned. Needs `EP_DISABLE_GIN=1` on NIC-less nodes. |
...
| `models/glm52/pd-vllm-prefill.md` | Decision record and completed gates for vLLM prefill → OpenInfer decode. Pegaflow #395 is pinned; Qwen3 vLLM 0.23.0 and GLM vLLM 0.24.0 use the normal layouts already gated against OpenInfer, so #382 is not a dependency. |
...
| `benchmarks/qwen3-14b-pd-vs-mix-h200.md` | Qwen3-14B 多轮长输入（8k+2k/轮）P/D 战役（8×H200）：等卡 P/D 吞吐反超 mixed +17% 且 ITL p99 23.5 vs 105ms；vLLM 0.25.1 吞吐/内核更快但 ITL p99 141ms；重负载下 200GiB hugepage host 池 vs 16GiB 决定 decode 纯净性（84.6→23.6ms）；2P+2D 弹性 ladder 证明 router 前缀亲和是必要条件（round-robin 85 → P+D 亲和 23.0ms、495 tok/s，pegaflow #405）。 |

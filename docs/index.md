...
| `models/kimi-k2/attention_flashinfer_mla_decode_report.md` | `decode.attention.flashinfer_mla_decode` H20 report：ctx-sensitive；`ctx=8192` 为 `103.5ms/step`，selected NCU 显示 `32` CTAs / `0.41` waves/SM / `254` regs/thread / `0.87%` DRAM，当前 Kimi wrapper 未走 FlashInfer `partition_kv`。 |
...
| `models/kimi-k2/pplx_swiglu_report.md` | `decode.moe.pplx_swiglu` H20 report：PPLX routed SwiGLU 为 compute/elementwise，trace replay p95 `12.66us/call`、`759.7us/step`，NCU `6784 x 256` / `55.4%` SM / `6.3%` DRAM，停止 standalone 调参。 |

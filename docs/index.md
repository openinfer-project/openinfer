...
| `models/glm52/ep4-gb300.md` | DP4/EP4 (`--moe-topo ep4`) on 4xGB300: 64 whole experts/rank (188 GiB weights/rank), second DeepEP shim instantiation, and a weight-only routed-expert chain (bf16×fp8 mma on the aligned recv layout — no act re-quant/relayout/remap) replacing the sm_90a DeepGEMM masked GEMMs. Bring-up green 2026-07-12: 4-bucket graph capture, greedy determinism, 32-slot serving c=32 `559 tok/s` / TPOT p50 `39.05ms` untuned. Needs `EP_DISABLE_GIN=1` on NIC-less nodes. |

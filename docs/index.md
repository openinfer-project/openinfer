...
| `models/kimi-k2/deepep-migration.md` | PPLX→DeepEP 迁移研究 (submodule `d4f41e4`)：decode path 用 elastic API `do_expand=True` + `do_cpu_sync=False`（worst-case 定形 → 可 graph capture，per-expert 对齐段直喂 grouped GEMM）；Marlin 是 adapter 不是重写（已有 GPU-side counts + sentinel 机制）；真正难点是 host 层（at::cuda::CUDAStream、NVRTC JIT 需 AOT 化、NVSHMEM 构建依赖）。下一步 H200 Python 原型验证 graph capture + decode 延迟。 |

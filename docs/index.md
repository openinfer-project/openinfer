...
| `models/glm52/native-mtp-accuracy.md` | Native MTP must consume the target model's final-normalized hidden, matching official vLLM rather than the pre-final-norm residual. The fix raises matched c8 mean accepted length from `1.753` to `3.725` versus official vLLM's `3.786`; a full-checkpoint production-path gate protects the boundary. |

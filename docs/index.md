...
| Path | TL;DR |
| --- | --- |
| `models/qwen3/serving-perf-5090.md` | Qwen3-4B vs vLLM 0.22.1 tuning record: batched step tail (#345), chunked prefill (`--max-prefill-tokens`, default 2048), bs8/16 bucket removal (cuBLAS split-K hole). Beats vLLM on TTFT everywhere and outright in overload (QPS16 1865 vs 1674 tok/s); remaining mid-band TPOT gap is prefill-roofline-bound. |

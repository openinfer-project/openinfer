...
| `models/kimi-k2/operator-todo.md` | Kimi-K2 算子清单：Marlin WNA16 与 MLA decode 主链已接端到端；CUDA Graph + fused qkv_a + MoE overlap + shared gate/up fused GEMM + routed scaled-add 后，H20 fixture output16 p99 `14.45ms`，synthetic output64 avg `14.63ms` / p99 `15.06ms`。 |
| `models/kimi-k2/vllm-path-comparison.md` | Kimi-K2 decode 路径对照：vLLM-style fused qkv_a、MoE shared/routed compute overlap、shared gate/up fused GEMM、routed scaled-add 已过 H20 gate；output64 avg/p50 稳在 `15ms` 内，p99 约 `15.06ms`。 |

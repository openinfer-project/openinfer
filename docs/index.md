...
| `projects/qwen3-model-crate.md` | Qwen3 model crate split ready for diff review: `pegainfer-qwen3-4b` owns Qwen3 config/weights/executor/scheduler/tests/benches/kernel plan; root sees generic `EngineHandle`; `ModelForward` is removed; decode length-limit now emits the final token; full 5090 clippy/e2e/bench snapshot passes |

...
| `models/glm52/whole-step-decode-graph.md` | PR5c execution record: whole-step CUDA graph (one launch/rank/step) + weight-only fp8 GEMV for all m=1 projections + grid-strided device-bounded MoE quant chain = **200 → 25.3 ms/step** on jz-38, 8-way free (~308 tok/s). nsys breakdown of the remaining ~3 ms gap to vLLM (#542); indexer oracle reference drift (#541). |

...
| `models/glm52/pd-m2-execution.md` | GLM P/D M2 执行记录：vLLM TP8 prefill + openinfer EP8 decode（无 MTP），跨节点 pegaflow P2P。2026-07-12 链路全通：输出与 vLLM 逐 token 一致、D 零 prefill（admit 即 suffix=1）、快压 TPOT p99 26ms。硬依赖 vLLM≥0.24 + fp8_ds_mla + D 端 page-first/vLLM 层名 + RoPE deinterleave fixup。剩余：验收压测 + 故障注入；开放风险 indexer rope（>2048）。 |

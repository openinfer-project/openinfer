...
| `models/qwen3/dspark-integration.md` | **Design-before-code** doc for integrating DeepSeek **DSpark** (Jun 2026 paper + DeepSpec repo): it's our DFlash drafter + a rank-256 **Markov head** (semi-autoregressive, +16–18% accepted length over DFlash, the draft-quality win) + a **confidence head** feeding a hardware-aware **prefix scheduler** (the systems/throughput win). Released `dspark_qwen3_4b_block7` has an identical backbone to our `Qwen3-4B-DFlash-b16` (only +4 tensors, `block_size=7`, anchor-first layout); **verify/accept/KV-transaction reused unchanged** — DSpark only changes *propose*. Recommended phasing: P1 Markov head (lossless in greedy, one-function change), P2 confidence-head static truncation, P3 hardware-aware scheduler (fights our bucketed CUDA-graph verify; design-first). |

...
multi-subtile mma via NEW pure-increment {4,1} table entries (UNMEASURED
placeholders — the GB300 first sweep owns replacing them). A .so predating
those entries (or any non-Blackwell arch) fails rows>8 closed with
CUDA_ERROR_INVALID_VALUE — surfaced here as KernelLaunchError, never silent.
"""

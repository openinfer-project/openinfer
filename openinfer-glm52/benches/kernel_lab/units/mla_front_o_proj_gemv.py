"""Adapter for mla_front.o_proj_gemv — the MLA-front o_proj fp8 weight-only GEMV.
...
rows routing: 1 -> m=1 CUDA-core GEMV (long-K: 1 row/warp on every arch); 2 ->
register tile (bit-parity with m=1); 4 -> register tile (o_proj is NOT in the
batch-4 mma table); 8 -> single-tile tensor-core mma {ksplit=16, ntiles=2} on
BOTH the Blackwell and Hopper tables; 16/32/64 (MTP span-mapped verify rows)
-> multi-subtile mma (BTILES = rows/8 column subtiles sharing each weight
packet, Blackwell-only measured GB300 picks: {8,2} at 16/32, {16,2} at 64)
behind KERNEL_LAB_GEMV_MMA_MULTI — set by default inside the harness
(loader.py), so rows=64 works here while production keeps the #812 register
tiles at 16/32/48 pending the A/B; knob-unset non-Blackwell rows>8 fail
closed with CUDA_ERROR_INVALID_VALUE — surfaced here as KernelLaunchError,
never silent. All mma
routes need caller-owned f32 k-slice scratch [ksplit, rows, n]. Rows are
deterministic per value but NOT bit-identical across values — the
torch-tolerance gate covers the reorder.
"""

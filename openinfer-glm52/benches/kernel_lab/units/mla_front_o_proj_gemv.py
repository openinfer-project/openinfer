...
packet, Blackwell-only {4,1} UNMEASURED placeholder — deliberately not the
batch-8 winner {16,2}: at rows>=16 the tensor-core work per weight byte
already scales with batch, so the long-k weight-bound GEMV starts shallow;
the first GB300 sweep owns the real pick) with NO register-tile fallback —
off-table/non-Blackwell fails closed with CUDA_ERROR_INVALID_VALUE. All mma

...
measured GB300 picks: {8,2} / {4,4} / {2,2}) behind KERNEL_LAB_GEMV_MMA_MULTI
— set by default inside the harness (loader.py), so rows=64 works here while
production keeps the #812 register tiles at 16/32/48 pending the A/B; rows=48
has no measured multi-mma entry and benchs the register tile. All mma routes
need caller-owned f32 k-slice scratch [ksplit, rows, n]. Rows are

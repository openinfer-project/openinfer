...
# Lab default: the batched-GEMV multi-subtile mma (rows 16/32/64) is gated in
# the production .cu behind KERNEL_LAB_GEMV_MMA_MULTI=0/unset so that mainline
# routing stays the #812 register tiles; inside the harness the measured path
# must be the default or rows=64 units fail closed and rows-16/32 baselines
# (swept with the mma) stop reproducing. A/B vs the register tiles: run with
# KERNEL_LAB_GEMV_MMA_MULTI=0.
os.environ.setdefault("KERNEL_LAB_GEMV_MMA_MULTI", "1")

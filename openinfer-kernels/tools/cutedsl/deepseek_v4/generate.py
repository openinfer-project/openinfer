...
def load_sm120_gemm_class(cutlass_root: Path):
    ...
    spec = importlib.util.spec_from_file_location("openinfer_cutedsl_sm120_dense_gemm", gemm_path)
...
def find_cutlass_root(repo_root: Path, explicit: str | None) -> Path:
    ...
    candidates.extend(
        [
            ...
            repo_root / "cutlass-upstream",
            repo_root / "openinfer-kernels/third_party/flashinfer/3rdparty/cutlass",
        ]
    )
    ...
    raise FileNotFoundError(
        "Could not find CUTLASS CuTe DSL examples. Set OPENINFER_CUTEDSL_CUTLASS_ROOT."
    )

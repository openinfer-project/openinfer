...
const GLM52_TRTLLM_MOE_CUBINS: &[(&str, &str, &str)] = &[
    (
        "kGlm52MoeFc1Tile8",
        "Bmm_E4m3_E4m3E4m3_Fp32_t128x8x128u2_s8_et64x8_m64x8x32_c1x1x1_rM_TN_transOut_noShfl_dsFp8_schPd4x2x2x3_bN_tma_tmaSf_rgTma_clmp_lbW2_dynB_sm100f.cubin",
        "4c77a8566d6cb07579f1076d70556a529d4e23c7a778a29f07896b1c969000c6",
    ),
    (
        "kGlm52MoeFc2Tile8",
        "Bmm_Bfloat16_E4m3E4m3_Fp32_t128x8x128u2_s8_et64x8_m64x8x32_c1x1x1_rM_TN_transOut_noShfl_dsFp8_schPd4x2x2x3_bN_rgTma_clmp_dynB_sm100f.cubin",
        "768cf4270f55900c987d34f036d7f302775887983fadc5d636e054a78be9cb71",
    ),
    (
        "kGlm52MoeFc1Tile64",
        "Bmm_E4m3_E4m3E4m3_Fp32_t128x64x128u2_s6_et64x64_m64x64x32_c1x1x1_rM_TN_transOut_noShfl_dsFp8_schPd4x2x2x3_bN_tma_tmaSf_rgTma_clmp_lbW4_dynB_sm100f.cubin",
        "3e56eb422873d17c8288859d72f78a52b5ee098dc61ea48d164fafd8a9b43be2",
    ),
    (
        "kGlm52MoeFc2Tile64",
        "Bmm_Bfloat16_E4m3E4m3_Fp32_t128x64x128u2_s6_et64x64_m64x64x32_c1x1x1_rM_TN_transOut_noShfl_dsFp8_schPd4x2x2x3_bN_rgTma_clmp_dynB_sm100f.cubin",
        "0fa35732def7bf858082cc6089235a4242109a28cce3ecc86fe18386e34bd142",
    ),
];

fn generate_glm52_trtllm_moe_cubins(root: &Path, out_dir: &Path) {
    let cubin_dir = root.join("cubin/glm52/trtllm_moe/cubins");
    let mut generated = String::from(
        "// Generated by openinfer-kernels/build.rs from checked-in FlashInfer cubins.\n\
         #pragma once\n\n",
    );
    for &(symbol, file_name, sha256) in GLM52_TRTLLM_MOE_CUBINS {
        let path = cubin_dir.join(file_name);
        println!("cargo:rerun-if-changed={}", path.display());
        let bytes = fs::read(&path).unwrap_or_else(|err| {
            panic!(
                "failed to read GLM5.2 TRTLLM MoE cubin {}: {err}",
                path.display()
            )
        });
        ...
        .expect("write cubin size");
        writeln!(
            &mut generated,
            "static constexpr const char* {symbol}Name = \"{file_name}\";\n\
             static constexpr const char* {symbol}Sha256 = \"{sha256}\";\n"
        )
        .expect("write cubin identity");
    }
    let output = out_dir.join("glm52_trtllm_moe_cubins.inc");
    fs::write(&output, generated).unwrap_or_else(|err| {
        panic!(
            "failed to write generated GLM5.2 TRTLLM MoE cubin header {}: {err}",
            output.display()
        )
    });
}
...
fn glm52_trtllm_moe_arch_args(normalized_sms: &[String], nvcc: &str) -> Option<Vec<String>> {
    let has_blackwell = normalized_sms
        .iter()
        .any(|sm| sm_numeric_prefix(sm).is_some_and(|n| (100..120).contains(&n)));
    (has_blackwell && nvcc_accepts_gencode(nvcc, "100f", "100f")).then(|| {
        vec![
            "-gencode".to_string(),
...
fn glm52_trtllm_moe_sources(root: &Path) -> Vec<PathBuf> {
    let flashinfer = root.join("third_party/flashinfer/csrc");
    [
    ...
    .into_iter()
    .map(|path| flashinfer.join(path))
    .collect()
}

fn is_glm52_trtllm_moe_source(root: &Path, path: &Path) -> bool {
    path.file_name()
        .is_some_and(|name| name == "glm52_trtllm_moe.cu")
        || path.starts_with(root.join("third_party/flashinfer/csrc"))
}
...
fn write_glm52_trtllm_moe_stub(out_dir: &Path) -> PathBuf {
    let stub_path = out_dir.join("glm52_trtllm_moe_stub.cu");
    fs::write(
        &stub_path,
        "#include <cuda.h>\n#include <cuda_runtime_api.h>\n#include <stdint.h>\n\
         extern \"C\" CUresult glm52_trtllm_moe_create(int, int, void**) { return CUDA_ERROR_NOT_SUPPORTED; }\n\
         extern \"C\" CUresult glm52_trtllm_moe_destroy(void*) { return CUDA_SUCCESS; }\n\
         extern \"C\" CUresult glm52_trtllm_moe_launch(void*, const uint16_t*, const float*, const float*, const uint8_t*, const float*, const uint8_t*, const float*, uint16_t*, int, cudaStream_t) { return CUDA_ERROR_NOT_SUPPORTED; }\n",
    )
...
fn main() {
    ...
    let glm52_trtllm_moe_arch = glm52_enabled
        .then(|| glm52_trtllm_moe_arch_args(&nvcc_sm_targets, &nvcc))
        .flatten();
    ...
    if glm52_enabled {
        ...
        generate_glm52_trtllm_moe_cubins(&crate_root(), &out_dir);
    }
    ...
    let mut cu_files: Vec<_> = csrc_files
        ...
        .filter_map(|path| {
            ...
            if file_name == "glm52_trtllm_moe.cu" && glm52_trtllm_moe_arch.is_none() {
                return None;
            }
        ...
        .collect();
    if glm52_trtllm_moe_arch.is_some() {
        let sources = glm52_trtllm_moe_sources(&root);
        for source in &sources {
            assert!(
                source.is_file(),
                "GLM5.2 TRTLLM MoE source is missing: {}",
                source.display()
            );
            println!("cargo:rerun-if-changed={}", source.display());
        }
        cu_files.extend(sources);
    }
    cu_files.sort();
    ...
    for cu_file in &cu_files {
        ...
        if is_glm52_trtllm_moe_source(&root, cu_file) {
            nvcc_args.extend(
                glm52_trtllm_moe_arch
                    .as_ref()
                    .expect("TRTLLM MoE sources require sm_100f")
                    .clone(),
            );
        ...
        if is_glm52_trtllm_moe_source(&root, cu_file) {
            let trtllm_moe = root.join("cubin/glm52/trtllm_moe/include");
            let trtllm_gen = trtllm_moe.join("flashinfer/trtllm/batched_gemm/trtllmGen_bmm_export");
            for dir in &flashinfer.cccl {
                nvcc_args.extend(["-I".to_string(), dir.to_string_lossy().to_string()]);
            }
            nvcc_args.extend(
                [
                    ...
                    "-DTLLM_GEN_GEMM_CUBIN_PATH=\"embedded\"",
                    "--compiler-options",
                    "-fvisibility=hidden",
                ]
                .map(str::to_string),
            ...
            for include in [
                out_dir.clone(),
                csrc_dir.join("glm52/trtllm_moe_compat"),
                trtllm_moe,
                trtllm_gen,
                flashinfer.include.clone(),
                flashinfer.csrc.clone(),
                flashinfer.csrc.join("nv_internal"),
                flashinfer.csrc.join("nv_internal/include"),
    ...
    if glm52_enabled && glm52_trtllm_moe_arch.is_none() {
        println!(
            "cargo:warning=No sm_100f target; GLM5.2 TRTLLM fused MoE compiles as a NOT_SUPPORTED stub"
        );
        let stub_path = write_glm52_trtllm_moe_stub(&out_dir);
        let obj_file = out_dir.join("glm52_trtllm_moe_stub_cuda.o");
        let mut nvcc_args = vec![
            "-c".to_string(),

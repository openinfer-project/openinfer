//! Safe wrapper over the GLM5.2 DeepEP elastic shim (csrc/glm52_deepep/).
//!
//! One [`Glm52DeepEp`] per rank, used from that rank's worker thread. All kernel
...
//! [`Glm52DeepEpInfo::expert_alignment`]; `psum_expert` holds the running aligned
//! offsets (exclusive form). Combine never applies router weights — the
//! caller weights expert outputs before [`Glm52DeepEp::decode_combine`].
...
use crate::ffi::{self, Glm52DeepEpInfo};
use crate::tensor::DeviceContext;

fn shim_error(what: &str) -> anyhow::Error {
    // Safety: glm52_deepep_last_error returns a valid thread-local C string.
    let message = unsafe { CStr::from_ptr(ffi::glm52_deepep_last_error()) };
...
/// Baked shim capacities (GLM5.2 single-node 8-rank config).
pub fn glm52_deepep_info() -> Glm52DeepEpInfo {
    let mut info = Glm52DeepEpInfo::default();
    // Safety: plain struct fill, no preconditions.
    unsafe { ffi::glm52_deepep_info(&raw mut info) };
    info
}
...
pub fn glm52_deepep_unique_id() -> Result<[u8; 128]> {
    ...
    let rc = unsafe { ffi::glm52_deepep_unique_id(id.as_mut_ptr()) };
    ensure!(rc == 0, shim_error("glm52_deepep_unique_id"));
    Ok(id)
}

/// Dispatch-side scratch, allocated once per rank and reused every layer.
pub struct Glm52DeepEpDispatchScratch {
...
impl Glm52DeepEpDispatchScratch {
    ...
    pub fn new_decode(ctx: &DeviceContext) -> Result<Self> {
        Self::new(ctx, glm52_deepep_info().decode_max_tokens_per_rank as usize)
    }
}
...
pub struct Glm52DeepEp {
    ctx: NonNull<ffi::Glm52DeepEpCtx>,
    info: Glm52DeepEpInfo,
}

// One context per rank thread; the shim has no thread-affine state beyond
// the thread-local error string.
unsafe impl Send for Glm52DeepEp {}
...
impl Glm52DeepEp {
    ...
    pub fn new(unique_id: &[u8; 128], num_ranks: usize, rank_idx: usize) -> Result<Self> {
        ...
        ensure!(rc == 0, shim_error("glm52_deepep_ctx_create"));
        Ok(Self {
            ctx: NonNull::new(ctx)
                .ok_or_else(|| anyhow!("glm52_deepep_ctx_create returned null"))?,
            info: glm52_deepep_info(),
        })
    }
    ...
    pub fn decode_dispatch(
        ...
        num_tokens: usize,
        scratch: &mut Glm52DeepEpDispatchScratch,
        ...
        shim_call!("glm52_deepep_decode_dispatch", unsafe {
            ffi::glm52_deepep_decode_dispatch(
                self.ctx.as_ptr(),
    ...
    pub fn decode_combine(
        ...
        shim_call!("glm52_deepep_decode_combine", unsafe {
            ffi::glm52_deepep_decode_combine(
                self.ctx.as_ptr(),
                stream.cu_stream().cast(),
                x_ptr as *const _,
                sm_ptr as *const i32,
...
impl Drop for Glm52DeepEp {
    ...
    fn drop(&mut self) {
        ...
        let rc = unsafe { ffi::glm52_deepep_ctx_destroy(self.ctx.as_ptr()) };
        if rc != 0 {
            eprintln!("{}", shim_error("glm52_deepep_ctx_destroy"));
        }
    }
}

//! GLM5.2 TP4 MoE: one phase-kernel chain per layer (union + gate/up + SiLU +
//! down + LL all-reduce) over 1/4-intermediate slices of all 257 experts
...
//! `csrc/glm52/glm52_moe_tp4.cu` and `docs/models/glm52/moe-tp4-low-latency.md`.
...
pub const GLM52_TP4_RANKS: usize = 4;
pub const GLM52_TP4_HIDDEN: usize = 6144;
pub const GLM52_TP4_TOPK: usize = 8;
/// Routed experts + the shared expert at bank index 256.
pub const GLM52_TP4_BANK_EXPERTS: usize = 257;
/// gate|up rows per expert per rank (2 x 2048/4).
pub const GLM52_TP4_SLICE_ROWS: usize = 1024;
/// Intermediate slice per rank (2048/4).
pub const GLM52_TP4_SLICE_I: usize = 512;
/// Worst-case active-expert union: 8 tokens x (topk + shared).
pub const GLM52_TP4_UNION_MAX: usize = GLM52_TP4_TOKENS * (GLM52_TP4_TOPK + 1);
/// Rows per TP4 layer step (= kTokens in the kernel): every rank passes all
/// 8 global rows (replicated activations).
pub const GLM52_TP4_TOKENS: usize = 8;
/// gate/up k-split factor (kernel-side kKsplitB).
pub const GLM52_TP4_KSPLIT_B: usize = 16;
...
pub const GLM52_TP4_RS_SLOT_PACKETS: usize =
    2 * GLM52_TP4_TOKENS * GLM52_TP4_RANKS * GLM52_TP4_HIDDEN;

/// f32 scratch length for gate|up k-split partials.
pub const GLM52_TP4_BPART_LEN: usize =
    GLM52_TP4_KSPLIT_B * GLM52_TP4_UNION_MAX * GLM52_TP4_TOKENS * GLM52_TP4_SLICE_ROWS;
/// f32 scratch length for per-union-entry router probabilities across rows.
pub const GLM52_TP4_GUPROB_LEN: usize = GLM52_TP4_UNION_MAX * GLM52_TP4_TOKENS;
/// f32 scratch length for per-expert down partials.
pub const GLM52_TP4_CPART_LEN: usize = GLM52_TP4_UNION_MAX * GLM52_TP4_TOKENS * GLM52_TP4_HIDDEN;
/// bf16 scratch length for the SiLU-combined intermediate.
pub const GLM52_TP4_UG_LEN: usize = GLM52_TP4_UNION_MAX * GLM52_TP4_TOKENS * GLM52_TP4_SLICE_I;
...
// (`Glm52Tp4Exchange::teardown_rendezvous`), so Drop's unmap never pulls a
// mapping from under a peer's in-flight kernel.
unsafe impl Send for Glm52Tp4LlBuffer {}
unsafe impl Sync for Glm52Tp4LlBuffer {}

impl Glm52Tp4LlBuffer {
    ...
    pub fn alloc(bytes: usize, device_ordinals: &[usize]) -> Result<Self> {
        ...
        .result()
        .map_err(|err| anyhow!("TP4 LL buffer alloc ({bytes} B) failed: {err}"))?;
        Ok(Self { vas })
    }
...
impl Drop for Glm52Tp4LlBuffer {
    fn drop(&mut self) {
        let _ = unsafe { ffi::glm52_moe_tp4_free_ll_cuda(self.vas[0] as *mut std::ffi::c_void) };
    }
}

/// Advance the TP4 step epoch by one — enqueue exactly once per decode step
/// (before any TP4 layer launch of that step; captured into the same graph).
pub fn glm52_moe_tp4_epoch_advance(
    ctx: &DeviceContext,
    epoch_dev: &mut CudaSlice<u64>,
) -> Result<()> {
    let (epoch_ptr, _g) = epoch_dev.device_ptr_mut(&ctx.stream);
    unsafe {
        ffi::glm52_moe_tp4_epoch_advance_cuda(
            epoch_ptr as *mut std::ffi::c_void,
            ctx.stream.cu_stream(),
        )
    }
    .result()
    .map_err(|err| anyhow!("glm52_moe_tp4_epoch_advance_cuda failed: {err}"))
}

/// Occupancy-derived default grid for the mma phase kernels (no longer a
/// correctness invariant — nothing grid-syncs).
pub fn glm52_moe_tp4_max_blocks() -> Result<usize> {
    let mut blocks: i32 = 0;
    unsafe { ffi::glm52_moe_tp4_max_blocks_cuda(&mut blocks) }
        .result()
        .map_err(|err| anyhow!("glm52_moe_tp4_max_blocks_cuda failed: {err}"))?;
    ensure!(blocks > 0, "TP4 MoE kernel has zero co-resident blocks");
    Ok(blocks as usize)
}

/// Per-rank state for one TP4 MoE layer launch. All buffers must be
...
pub struct Glm52MoeTp4Buffers<'a> {
    ...
    /// Own LL buffer (`Glm52Tp4LlBuffer` address; sized
    /// slots x `GLM52_TP4_RS_SLOT_PACKETS` x 16 B).
    pub rs_local: u64,
    pub peer_rs: [u64; GLM52_TP4_RANKS],
...
/// Launch one layer's TP4 MoE phase-kernel chain. Every rank passes ALL
/// `GLM52_TP4_TOKENS` rows of `normed2`/`topk_*` (replicated activations —
...
/// have been advanced once this step via [`glm52_moe_tp4_epoch_advance`].
#[allow(clippy::too_many_arguments)]
pub fn glm52_moe_tp4_layer_launch(
    ctx: &DeviceContext,
    layer_slot: usize,
    ...
    bufs: &mut Glm52MoeTp4Buffers<'_>,
    myrank: usize,
    grid_blocks: usize,
) -> Result<()> {
    const H: usize = GLM52_TP4_HIDDEN;
    const E: usize = GLM52_TP4_BANK_EXPERTS;
    ensure!(myrank < GLM52_TP4_RANKS, "TP4 myrank {myrank} out of range");
    let rows = GLM52_TP4_TOKENS;
    ensure!(
        normed2.len() >= rows * H && mlp_out.len() >= rows * H,
        "TP4 hidden buffers too small: normed2 {}, mlp_out {}",
        normed2.len(),
        mlp_out.len()
    );
    ensure!(
        topk_idx.len() >= rows * GLM52_TP4_TOPK && topk_prob.len() >= rows * GLM52_TP4_TOPK,
        "TP4 topk buffers too small: idx {}, prob {}",
        topk_idx.len(),
        topk_prob.len()
    );
    ensure!(
        w13.len() == E * GLM52_TP4_SLICE_ROWS * H
            && w13_scale.len() == E * (GLM52_TP4_SLICE_ROWS / 128) * (H / 128)
            && w2.len() == E * H * GLM52_TP4_SLICE_I
            && w2_scale.len() == E * (H / 128) * (GLM52_TP4_SLICE_I / 128),
        "TP4 weight slice shape mismatch: w13 {} scale {} w2 {} scale {}",
        w13.len(),
        w13_scale.len(),
        w2.len(),
        w2_scale.len()
        ...
        bufs.guidx.len() >= GLM52_TP4_UNION_MAX
            && bufs.guprob.len() >= GLM52_TP4_GUPROB_LEN
            && !bufs.gucnt.is_empty()
            && bufs.gused.len() >= 256
            && bufs.bpart.len() >= GLM52_TP4_BPART_LEN
            && bufs.ug.len() >= GLM52_TP4_UG_LEN
            && bufs.cpart.len() >= GLM52_TP4_CPART_LEN
            && !bufs.epoch_dev.is_empty(),
        "TP4 scratch arena too small"
    );
    ensure!(grid_blocks > 0, "TP4 grid_blocks must be positive");
    ensure!(
        bufs.rs_local != 0 && bufs.peer_rs.iter().all(|&p| p != 0),
        "TP4 LL pointers not wired"
    );
    ...
    let peer_rs: [*const std::ffi::c_void; GLM52_TP4_RANKS] =
        bufs.peer_rs.map(|p| p as *const std::ffi::c_void);
    unsafe {
        ffi::glm52_moe_tp4_layer_launch_cuda(
    ...
    .result()
    .map_err(|err| anyhow!("GLM5.2 TP4 MoE layer launch failed: {err}"))
}

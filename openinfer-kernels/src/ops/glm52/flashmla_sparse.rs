...
impl Glm52FlashMlaSparseDecode {
    pub fn validate(self) -> Result<()> {
            ...
            self.num_sm_parts > 0,
            "GLM5.2 FlashMLA sparse decode num_sm_parts must be positive"
        );
        ensure!(
...
pub fn glm52_flashmla_sparse_decode_num_sm_parts() -> Result<usize> {
    ...
    result
        ...
        num_sm_parts > 0,
        "GLM5.2 FlashMLA sparse num_sm_parts query returned {num_sm_parts}"
    );
...
pub fn glm52_flashmla_sparse_decode_metadata_launch(
    ...
    contract.validate()?;
    validate_metadata_buffers(contract, topk_length, tile_scheduler_metadata, num_splits)?;

    let topk_length_guard;
    let topk_length_ptr = if let Some(topk_length) = topk_length {
        let (ptr, guard) = topk_length.device_ptr(&ctx.stream);
        topk_length_guard = Some(guard);
        ptr as *const i32
    } else {
        topk_length_guard = None;
        std::ptr::null()
    };
...
fn validate_metadata_buffers(
    ...
    if let Some(topk_length) = topk_length {
            ...
            topk_length.len() >= contract.batch_size,
            "GLM5.2 FlashMLA sparse topk_length too small: have {}, need {}",
            topk_length.len(),
            contract.batch_size
        );
    }

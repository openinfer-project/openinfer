//! GLM5.2 TP4 attention allreduce: the o_proj epilogue
...
//! width); the kernel pair sums the 4 partials in a fixed order so every rank
...
use super::moe_tp4::{GLM52_TP4_HIDDEN, GLM52_TP4_RANKS, GLM52_TP4_TOKENS};

/// LL packets per (row, src) hidden chunk: H/8 bf16 packed 6 (12 B) per
/// packet. This is also the pre-offset unit for peer pointers.
pub const GLM52_TP4_AR_CHUNK_PACKETS: usize = (GLM52_TP4_HIDDEN / GLM52_TP4_RANKS) * 2 / 12;
...
pub const GLM52_TP4_AR_SLOT_PACKETS: usize =
    2 * 2 * GLM52_TP4_TOKENS * GLM52_TP4_RANKS * GLM52_TP4_AR_CHUNK_PACKETS;

/// AR LL buffer bytes for `layer_slots` layers (allocate via
/// [`super::moe_tp4::Glm52Tp4LlBuffer::alloc`]).
pub const fn glm52_tp4_ar_buffer_bytes(layer_slots: usize) -> usize {
    layer_slots * GLM52_TP4_AR_SLOT_PACKETS * 16
}
...
/// `myrank * GLM52_TP4_AR_CHUNK_PACKETS` packets (this rank's src slot).
/// Shares the step epoch with the MoE chain:
/// [`super::moe_tp4::glm52_moe_tp4_epoch_advance`] once per step.
...
pub fn glm52_tp4_ar_launch(
    ...
    ensure!(myrank < GLM52_TP4_RANKS, "AR myrank {myrank} out of range");
    ensure!(
        rows >= 1 && rows <= GLM52_TP4_TOKENS,
        "AR rows {rows} out of 1..={GLM52_TP4_TOKENS}"
    );
    ensure!(
        partial.len() >= rows * GLM52_TP4_HIDDEN && out.len() >= rows * GLM52_TP4_HIDDEN,
    ...
    let peer_ar: [*const std::ffi::c_void; GLM52_TP4_RANKS] =
        peer_ar.map(|p| p as *const std::ffi::c_void);
    unsafe {
        ffi::glm52_tp4_ar_launch_cuda(
    ...
    .result()
    .map_err(|err| anyhow!("GLM5.2 TP4 attention allreduce launch failed: {err}"))
}

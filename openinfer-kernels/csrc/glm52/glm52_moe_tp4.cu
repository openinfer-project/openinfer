// GLM5.2 TP4 MoE: one short chain of phase kernels per layer replacing the
// EP8 dispatch/grouped-GEMM/combine chain
// (docs/models/glm52/moe-tp4-low-latency.md).
//
// Topology: every rank holds a 1/4 intermediate-slice of ALL 257 experts
...
constexpr int kSliceRows = 1024;               // gate|up rows per expert per rank
constexpr int kSliceI = 512;                  // intermediate slice (2048/4)
...
// measured — the TP4 kernel's own wall stays innocent at ~56 us). The fence
...
__global__ void tp4_epoch_advance_kernel(unsigned long long* epoch_dev) {
  *epoch_dev += 1;
}
...
__global__ void __launch_bounds__(kThreads) tp4_union_kernel(
    Glm52MoeTp4Args a) {
  TP4_PREAMBLE
...
__global__ void __launch_bounds__(kThreads) tp4_gemm_b_kernel(
    Glm52MoeTp4Args a) {
  TP4_PREAMBLE
  const int w = t >> 5;
...
__global__ void __launch_bounds__(kThreads) tp4_silu_kernel(Glm52MoeTp4Args a) {
  TP4_PREAMBLE
  const int UC = *a.gucnt;
...
__global__ void __launch_bounds__(kThreads) tp4_gemm_c_kernel(
    Glm52MoeTp4Args a) {
  TP4_PREAMBLE
  const int w = t >> 5;
  const int gw = blockIdx.x * (kThreads / 32) + w, TW = gridDim.x * (kThreads / 32);
...
__global__ void __launch_bounds__(kThreads) tp4_rs_push_kernel(
    Glm52MoeTp4Args a) {
  TP4_PREAMBLE
  const int UC = *a.gucnt;
...
__global__ void __launch_bounds__(kThreads) tp4_rs_recv_kernel(
    Glm52MoeTp4Args a) {
  TP4_PREAMBLE
...
cudaError_t tp4_occupancy_grid(const void* kernel_fn, int* out_blocks) {
...
int glm52_moe_tp4_max_blocks_cuda(int* out_blocks) {
  return (int)tp4_occupancy_grid((const void*)tp4_gemm_b_kernel, out_blocks);
}
...
LlVmmRecord g_ll_vmm_tp4[64] = {};
int g_ll_vmm_tp4_count = 0;
std::mutex g_ll_vmm_tp4_mu;  // rank threads allocate concurrently
}

int glm52_moe_tp4_alloc_ll_cuda(size_t bytes, const int* device_ordinals,
                                int n_devices, unsigned long long* out_vas) {
  int dev = 0;
  cudaError_t e = cudaGetDevice(&dev);
  if (e != cudaSuccess) return (int)e;
  std::lock_guard<std::mutex> lock(g_ll_vmm_tp4_mu);
  if (n_devices <= 0 || n_devices > kLlMaxDevices || g_ll_vmm_tp4_count >= 64) {
  ...
  g_ll_vmm_tp4[g_ll_vmm_tp4_count++] = rec;
  return 0;
...
int glm52_moe_tp4_free_ll_cuda(void* p) {
  std::lock_guard<std::mutex> lock(g_ll_vmm_tp4_mu);
  for (int i = 0; i < g_ll_vmm_tp4_count; ++i) {
    bool hit = false;
    for (int j = 0; j < g_ll_vmm_tp4[i].n_vas; ++j) {
      hit |= g_ll_vmm_tp4[i].vas[j] == (CUdeviceptr)p;
    }
    if (!hit) continue;
    for (int j = 0; j < g_ll_vmm_tp4[i].n_vas; ++j) {
      (void)cuMemUnmap(g_ll_vmm_tp4[i].vas[j], g_ll_vmm_tp4[i].size);
      (void)cuMemAddressFree(g_ll_vmm_tp4[i].vas[j], g_ll_vmm_tp4[i].size);
    }
    (void)cuMemRelease(g_ll_vmm_tp4[i].handle);
    g_ll_vmm_tp4[i] = g_ll_vmm_tp4[--g_ll_vmm_tp4_count];
    return 0;
  }
  return (int)cudaErrorInvalidValue;
}

int glm52_moe_tp4_layer_launch_cuda(
  ...
  {
    cudaError_t e =
        tp4_occupancy_grid((const void*)tp4_gemm_c_kernel, &gemm_c_blocks);
    if (e != cudaSuccess) return (int)e;
  }
  void* args[] = {&a};
  ...
  struct Phase {
    ...
    int blocks;
      ...
      {(const void*)tp4_union_kernel, 1},
      {(const void*)tp4_gemm_b_kernel, grid_blocks},
      {(const void*)tp4_silu_kernel, grid_blocks},
      {(const void*)tp4_gemm_c_kernel, gemm_c_blocks},
      {(const void*)tp4_rs_push_kernel, rs_blocks * kTokens},
      {(const void*)tp4_rs_recv_kernel, rs_blocks * kTokens},
  };
...
// Step-head epoch advance: exactly one node per replayed step (all TP4
// layers of the step share the epoch; per-layer slot regions alternate
// parity across steps).
int glm52_moe_tp4_epoch_advance_cuda(void* epoch_dev, cudaStream_t stream) {
  void* args[] = {&epoch_dev};
  return (int)cudaLaunchKernel((const void*)tp4_epoch_advance_kernel, dim3(1),

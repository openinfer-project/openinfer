...
CUresult current_sm90_num_sm_parts(int* num_sm_parts) {
  ...
  *num_sm_parts = std::max(prop.multiProcessorCount / kSq / (kHeads / 64), 1);
  return CUDA_SUCCESS;
}
...
extern "C" CUresult glm52_flashmla_sparse_decode_metadata_cuda(
    const int* topk_length, int* tile_scheduler_metadata, int* num_splits,
    int batch_size, int topk, int num_sm_parts, cudaStream_t stream) {
...
extern "C" CUresult glm52_flashmla_sparse_decode_launch_cuda(
    ...
    const int* topk_length, const int* tile_scheduler_metadata,
    const int* num_splits, void* out_latent, float* lse, float* lse_accum,
    float* o_accum, int batch_size, int num_blocks, int topk, int num_sm_parts,
  ...
  CUresult result = consume_last_cuda_error();

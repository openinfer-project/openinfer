...
// (active rows only).
__global__ void __launch_bounds__(kThreads) tp4_ar_push_kernel(
    Glm52Tp4ArArgs a) {
  TP4_AR_PREAMBLE
...
__global__ void __launch_bounds__(kThreads) tp4_ar_reduce_bcast_kernel(
    Glm52Tp4ArArgs a) {
  TP4_AR_PREAMBLE
...
__global__ void __launch_bounds__(kThreads) tp4_ar_recv_kernel(
    Glm52Tp4ArArgs a) {
  TP4_AR_PREAMBLE
...
extern "C" int glm52_tp4_ar_launch_cuda(
  ...
  tp4_ar_push_kernel<<<push_blocks, kThreads, 0, stream>>>(a);
  tp4_ar_reduce_bcast_kernel<<<reduce_blocks, kThreads, 0, stream>>>(a);
  tp4_ar_recv_kernel<<<push_blocks, kThreads, 0, stream>>>(a);

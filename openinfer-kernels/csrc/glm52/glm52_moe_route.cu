...
__global__ void glm52_moe_route_offsets_kernel(const int* __restrict__ topk_idx,
  ...
  for (int e = 0; e < n_experts; ++e) {
    ...
    for (int j = 0; j < topk; ++j) {
      if (topk_idx[j] == e) ++count;
    }
    int start = (running + alignment - 1) / alignment * alignment;
    expert_offsets[e] = static_cast<long long>(start);
    running = start + count;
  }
  expert_offsets[n_experts] =
      static_cast<long long>((running + alignment - 1) / alignment * alignment);
}
...
CUresult glm52_moe_route_offsets_cuda(const int* topk_idx, long long* expert_offsets,
  ...
  glm52_moe_route_offsets_kernel<<<1, 1, 0, stream>>>(topk_idx, expert_offsets,
                                                      n_experts, topk, alignment);

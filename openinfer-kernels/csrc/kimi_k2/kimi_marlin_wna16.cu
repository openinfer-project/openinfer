...
CUresult kimi_marlin_wna16_gemm_cuda(
    ...
    int sm_count,
    cudaStream_t stream) {
  return openinfer_kimi_marlin_moe_wna16::launch_marlin_gemm(
...
CUresult kimi_marlin_w13_swiglu_cuda(
  ...
  openinfer_kimi_marlin_moe_wna16::swiglu_w13_kernel<<<blocks, threads, 0, stream>>>(
      w13, out, rows, intermediate_dim);
  return openinfer_kimi_marlin_moe_wna16::last_error_to_cu(cudaPeekAtLastError());
}
...
CUresult kimi_marlin_w13_swiglu_expanded_cuda(
  ...
  openinfer_kimi_marlin_moe_wna16::swiglu_w13_expanded_kernel<<<blocks, threads, 0, stream>>>(
...
CUresult kimi_marlin_sum_topk_rows_f32_cuda(
  ...
  int total = active_tokens * hidden_dim;
  int blocks = (total + threads - 1) / threads;
  openinfer_kimi_marlin_moe_wna16::sum_topk_rows_kernel<<<blocks, threads, 0, stream>>>(

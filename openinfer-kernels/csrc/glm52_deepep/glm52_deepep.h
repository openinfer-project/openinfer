// C ABI for the DeepEP elastic EP shim (GLM5.2 single-node 8-rank config).
//
// AOT replacement for DeepEP's NVRTC JIT + torch host layer: the kernel
// template specializations for the GLM5.2 config are instantiated at build
// time (glm52_deepep_shim.cu) and launched through cudaLaunchKernelExC with the
...
// message is available via glm52_deepep_last_error() (thread-local).
#pragma once
...
typedef struct {
    ...
    // Actual counts come back through glm52_deepep_prefill_wait_counts.
...
// All outputs caller-allocated at the worst-case sizes from Glm52DeepEpInfo.

...
// (csrc/elastic/buffer.hpp) for the GLM5.2 single-node 8-rank config. The
// kernel headers under deep_ep/include are torch-free; everything above them
// is reimplemented here against the C ABI in glm52_deepep.h:
...
//
// Warp counts and capacities come from glm52_deepep_config.cuh; ctx_create
...
#include "glm52_deepep.h"
#include "glm52_deepep_config.cuh"

namespace {

using namespace deep_ep;
using namespace deep_ep::elastic;
namespace cfg = deepep_glm52_shim::cfg;

// The C ABI hands top-k indices over as i32; build with -DEP_NUM_TOPK_IDX_BITS=32. DeepEP's wire format stores
...
#define SHIM_API_END                  \
    return 0;                         \
        ...
        set_last_error("glm52 deepep shim: unknown error"); \
        return -1;                    \
    }

void check_cuda(cudaError_t err, const char* what) {
    if (err != cudaSuccess)
        throw std::runtime_error(std::string("glm52 deepep shim: ") + what + ": " +
...
void require(bool cond, const char* what) {
    if (!cond) throw std::runtime_error(std::string("glm52 deepep shim: ") + what);
}
...
void launch_barrier_on(Glm52DeepEpCtx* ctx, cudaStream_t stream) {
...
layout::WorkspaceLayout host_workspace_layout(Glm52DeepEpCtx* ctx) {
...
void dispatch_send_impl(Glm52DeepEpCtx* ctx, cudaStream_t stream, const void* x,
...
template <typename Path>
void copy_epilogue_impl(Glm52DeepEpCtx* ctx, cudaStream_t stream, int32_t num_recv_tokens,
...
template <typename Path>
void combine_impl_call(Glm52DeepEpCtx* ctx, cudaStream_t stream, const void* x,
...
const char* glm52_deepep_last_error(void) { return g_last_error.c_str(); }

void glm52_deepep_info(Glm52DeepEpInfo* out) {
...
int glm52_deepep_ctx_create(const uint8_t unique_id[128], int32_t num_ranks, int32_t rank_idx,
    ...
    auto ctx = std::make_unique<Glm52DeepEpCtx>();
...
int glm52_deepep_decode_dispatch(Glm52DeepEpCtx* ctx, void* stream, const void* x,
                           const int32_t* topk_idx, const float* topk_weights,
                           int32_t num_tokens, int32_t* rank_count_scratch,
                           int32_t* dst_slot_scratch, int32_t* psum_rank,
...
int glm52_deepep_decode_combine(Glm52DeepEpCtx* ctx, void* stream, const void* x,
                          const int32_t* src_metadata, const int32_t* psum_rank,
...
int glm52_deepep_prefill_dispatch_send(Glm52DeepEpCtx* ctx, void* stream, const void* x,
                                 const int32_t* topk_idx, const float* topk_weights,
                                 int32_t num_tokens, int32_t* rank_count_scratch,
                                 int32_t* dst_slot_scratch, int32_t* psum_rank,
                                 int32_t* psum_expert) {
...
int glm52_deepep_prefill_wait_counts(Glm52DeepEpCtx* ctx, int32_t* num_recv_tokens,
    ...
    while (rank_i < cfg::kNumRanks || expert_i < cfg::kNumLocalExperts) {
        ...
        if (!ready &&
            std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() -
                                                             start)
                    .count() > 120)
            throw std::runtime_error("glm52 deepep shim: prefill dispatch CPU wait timed out");
    }
...
int glm52_deepep_prefill_dispatch_recv(Glm52DeepEpCtx* ctx, void* stream, int32_t num_recv_tokens,
...
int glm52_deepep_prefill_combine(Glm52DeepEpCtx* ctx, void* stream, const void* x,
                           const int32_t* src_metadata, const int32_t* psum_rank,

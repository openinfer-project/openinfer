...
impl Qwen35Model {
    ...
    pub(crate) fn from_safetensors_with_device_options_and_reservation(
        ...
        let effective_bytes_per_page = bytes_per_page.saturating_add(dflash_per_page);
        let available = free_bytes
            .saturating_sub(scratch_reserve)
            .saturating_sub(dflash_fixed_reserve);
        ...
        let dflash_fixed_mb = dflash_fixed_reserve / (1024 * 1024);
        info!(
            "Qwen3.5 KV cache: {num_pages} pages ({kv_mb} MB), prefill scratch reserve: {scratch_mb} MB, DFlash fixed reserve: {dflash_fixed_mb} MB, {:.0}% of {:.0} MB free",
    ...
    pub(crate) fn output_projection_tied(&self) -> &DeviceMatrix {
        &self.embed_tokens
    }

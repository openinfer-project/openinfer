...
///   decode batch.
///
/// The scratch accounting intentionally stays conservative: the live draft path
/// allocates one lane-level `DFlashBatchScratch` for dense block rows, while the
/// durable request cache still keeps per-request prefix and tail buffers. Keeping
/// `tail_scratch` in both the pool-scaled term and the one-block headroom term
/// slightly over-reserves memory, but it prevents late OOM when admission accepts
/// a request close to the context limit.
